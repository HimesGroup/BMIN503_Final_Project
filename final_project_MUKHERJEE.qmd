---
title: "determinants of liver steatosis"
...
Supervised Machine Learning using Random Forest Model to assess the most important factors To Assess GINI
...
```{r}
target_column_name <-  "body_bmi"
column_number <- which(names(df) == target_column_name)
...
ggplot(data=df, aes(x = fibroscan_cirrhosis, y = body_bmi)) +
...
ggplot(df, aes(x = body_bmi, y = fibroscan_meanstiffness_kpa, color=hcrna)) + geom_point()+
  geom_smooth(method = "lm") 

#PLOT to see the numebr of hb
ggplot(df, aes(x = body_bmi, y = fibroscan_meanstiffness_kpa, color=hbcore_positive)) + geom_point()+
  geom_smooth(method = "lm") 
...
autoplot(roc_curve(rf.predicted.cirrhosis, 
                   truth, .pred_Cases ))

roc_auc(rf.predicted.cirrhosis, 
                   truth,  .pred_Cases )
