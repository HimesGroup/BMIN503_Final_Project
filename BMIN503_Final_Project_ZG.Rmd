...
general health (health_gen)
vaginal deliveries (repro_children)
sexually active (sex_active)
LURN-SI 29 score (lurnsi29, NEED TO CREATE)
physical activity level (ipaq, NEED TO CREATE)
...
```{r}                         
#Create a variable list to use for Table 1, including both LURN SI-29 score (all urinary symptoms) and CASUS score (incontinence symptoms only). Then create lists for categorical and numeric variables.
listVarsT1 <- c("age", "bmi", "edu", "race", "hisp", "cig_hx", "insurance", "health_gen", "sex_hx", "vag_del", "lurnsi29cat", "casus_total_cat", "ipaq")
catVarsT1 <- c("edu", "race", "hisp", "cig_hx", "insurance", "health_gen", "sex_hx", "vag_del", "lurnsi29cat", "casus_total_cat", "ipaq")

#Create a prelim table 1 for overall cohort characteristics. 
table1pre <- CreateTableOne(vars = listVarsT1, data = bladderhealth_mv.df, factorVars = catVarsT1)
print(table1pre, varLabels = TRUE)

#Assess missingness and normality of variables.
summary(table1pre) 
#All variables missing <20 values. Age is not highly skewed (skew 0.3 < 1), while BMI is skewed (skew 1.4 > 1); will use mean/SD for age and median/IQR for BMI.
nonnormVars <- "bmi"

#Stratify Table 1 by binary outcome groups (doing Kegels several times a month or more, vs not). This package uses Chi-square test for comparisons between categorical variables and one-way ANOVA for continuous variables.
table1 <- CreateTableOne(listVarsT1, bladderhealth_mv.df, catVarsT1, strata = c("kegel_89_binary"))
print(table1, varLabels = TRUE)
```

Compared to women who are not doing Kegels, women doing Kegels several times a month or more are significantly older (P = 0.042), have more of a smoking history (i.e. smoked more than 100 cigarettes throughout their lifetime, P = 0.003), are less insured (P = 0.005), have a history of sexual activity (P = 0.040), have worse urinary symptoms based on LURN SI-29 scores (P = 0.020), and have higher physical activity level based on IPAQ score (P = 0.003). 
...
univar_mv <- tbl_merge(tbls = list(univar, mv), tab_spanner = c("**Univariate**", "**Multivariable**"))
univar_mv
...
#Does the proportion of women learning about Kegels from a doctor, nurse, or therapist (kegel_92___4) differ between insured and uninsured?
table(bladderhealth_mv.df$kegel_92___4, bladderhealth_mv.df$insurance)
prop.table(table(bladderhealth_mv.df$kegel_92___4, bladderhealth_mv.df$insurance), 2) 
#Yes, 36.4% of insured vs. 26.7% of uninsured.

#Explore relationship between insurance and other variables that significantly predicted Kegel frequency on univariate analysis. 

#Are the insured women younger and therefore less likely to do Kegels?
bladderhealth_mv2.df <- bladderhealth_mv.df %>% filter(!is.na(insurance))
#Visually compare age distributions between insured and uninsured.
ggplot(bladderhealth_mv2.df, aes(x = age)) + geom_histogram(binwidth = 1) + facet_wrap(~insurance) 
#Insured women appear to be relatively more concentrated in their 30s compared to uninsured. Distributions do not appear normal.
...
#Insured women are actually older.

#Are uninsured women more likely to be physically active, smoke, have a history of sexual activity, or have severe urinary symptoms?
...
#Uninsured women more likely to be in low activity group (55.9%, vs. 46.5% of insured women).
...
#Uninsured women more likely to smoke >100 cigarettes in their lifetime (39.1%, vs. 28.8% of insured women).
...
#Uninsured women have more severe urinary symptoms; 38.5% in high symptom group vs. only only 31.4% for insured women).

```

Uninsured women are more likely to have lower activity levels, smoke, and have severe urinary symptoms. While having more severe urinary symptoms may lead to them doing Kegels more often, in theory the multivariable regression should account for this. Will plan to discuss this more in final version of results section.
