...
```{r data_prep, echo = TRUE, cache = TRUE, message=FALSE}
library(dplyr)
library(moments)
library(ggplot2)
...
ctrl <- df %>% 
  filter(TBI!=1) %>%
  filter(!PTID %in% icd.sub)                # removes subjecrs with other pathologies from control group
...
roi40 <- roi[1:40, "ROI"]
top40 <- muse.dict[muse.dict$ROI%in%roi40, "ROI_NAME_INDEX"]
```
...
plot_roi_icv <- function(data, inp) {
  ggplot(data, aes(inp)) +
                ylim(0, 15) +
                labs(title = "Histograms: z-scored ICV-corrected ROI volumes in TBI case subjects", x= paste0("z-score of ",inp)) +
                geom_histogram(breaks = seq(-3.5, 3.5, .5), color = "midnightblue", fill = "midnightblue", alpha = 0.7)}

plot_roi <- function(inp) {
  ggplot(df.z, aes(inp)) + 
                ylim(0, 15) +
                labs(title = "Histograms: z-scored absolute ROI volumes in TBI case subjects", x= paste0("z-score of ",inp)) +
                geom_histogram(breaks = seq(-3.5, 3.5, .5), color = "blue", fill = "blue", alpha = 0.7)}

print("Top 40 ROIs with Most Outliers among Case Subjects")
roi40

ggplot(df.z.icv, aes(MUSE_Volume_414_z)) +
                ylim(0, 15) +
                labs(title = "Histograms: z-scored ICV-corrected ROI volumes in TBI case subjects", x= paste0("z-score of ICV-corrected Deep Gray Matter (left hemisphere)")) +
                geom_histogram(breaks = seq(-3.5, 3.5, .5), color = "midnightblue", fill = "midnightblue", alpha = 0.7)

ggplot(df.z, aes(MUSE_Volume_414_z)) + 
                ylim(0, 15) +
                labs(title = "Histograms: z-scored absolute ROI volumes in TBI case subjects", x= paste0("z-score of Deep Gray Matter (left hemisphere)")) +
                geom_histogram(breaks = seq(-3.5, 3.5, .5), color = "blue", fill = "blue", alpha = 0.7)

ggplot(df.z.icv, aes(MUSE_Volume_60_z)) +
                ylim(0, 15) +
                labs(title = "Histograms: z-scored ICV-corrected ROI volumes in TBI case subjects", x= paste0("z-score of ICV-corrected Left Thalamus")) +
                geom_histogram(breaks = seq(-3.5, 3.5, .5), color = "midnightblue", fill = "midnightblue", alpha = 0.7)

ggplot(df.z, aes(MUSE_Volume_60_z)) + 
                ylim(0, 15) +
                labs(title = "Histograms: z-scored absolute ROI volumes in TBI case subjects", x= paste0("z-score of Left Thalamus")) +
                geom_histogram(breaks = seq(-3.5, 3.5, .5), color = "blue", fill = "blue", alpha = 0.7)
...
```{r results_2, echo = FALSE, cache = TRUE, message=FALSE}
df.z.o <- df.z
df.z.icv.o <- df.z.icv
...
outlier_function <- function(inp) {
  ...
  o_pos <- sum(outliers_positive)
  o_neg <- abs(sum(outliers_negative))
  o <- o_neg + o_pos
  N_sbj <- nrow(inp)
  p_pos <- o_pos/N_sbj
  p_neg <- o_neg/N_sbj
  ...
  out <- format(c(N_sbj, p_pos, p_neg, p), scientific=FALSE,drop0trailing = TRUE)
  return(out)
}
...
names <- c("N", "z-scores greater than 2, subject average","z-scores less than 2, subject average", "z-scores outside +/- 2, subject average")
