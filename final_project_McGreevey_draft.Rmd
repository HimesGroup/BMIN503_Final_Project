...
Lowaccess_county<- Lowaccess %>% group_by(COUNTYFIPS) %>% summarize(MEAN_PCT_LA_COUNTY = mean(FARA_PCT_LA_HALF_10_POP))
...
ggplot(data = test_leftjoin, aes(fill = MEAN_PCT_LA_COUNTY, lwd = 0)) +
  geom_sf(aes(geometry = geometry))+
  my_theme() +
  ggtitle("Percent of population with low access to food, by U.S. county, 2015") +
  scale_fill_continuous(low = "antiquewhite2", high = "palevioletred4", guide = "colorbar")+
  theme(legend.position = "none")

#Low access plot:
ggplot() +
  geom_sf(data = test_leftjoin, aes(fill = 
...
#Diabetes plot:
ggplot() +
  geom_sf(data = test_leftjoin, aes(fill = CHR_PCT_DIABETES), lwd = 0) +
  my_theme() +
