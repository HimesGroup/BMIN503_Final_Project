...
```{r eval = TRUE}
head(dplyr::select(Lowaccess_raw, TRACTFIPS, COUNTYFIPS, STATEFIPS, STATE, COUNTY, REGION, ACS_TOT_POP_WT, FARA_PCT_LA_HALF_10_POP)
Countyfips
Statefips
State
County
Region
ACS_Tot_pop_wt))
head(dplyr::select(nhanes, age:education)) #Can use names for ranges with select function
head(dplyr::select(nhanes, -(age:race))) #Can exclude ranges with "-"
nhanes <- dplyr::select(nhanes, c(id:education, adult.status)) #Saving an abbreviated table, note replacing of original should be done with care
