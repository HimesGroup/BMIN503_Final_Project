...
fulluniquevisits<-as_tibble(fullmerge[!duplicated(fullmerge[c(1,2,3,54)]),])
...
distinctRXrecips<-as_tibble(RXrecips [!duplicated(RXrecips[c(2,4,6,54)]),])
s<-dim(distinctRXrecips)

#create unique RX recipient families - returns 3598 (on Plast, Phone,Clast)
distinctRXrecipfams<-as_tibble(distinctRXrecips [!duplicated(distinctRXrecips[c(4,6,54)]),])
...
distinctRXredemps<-as_tibble(RXredemps [!duplicated(RXredemps[c(2,4,6,54)]),])
v<-dim(distinctRXredemps)

#create distinct RX redemption families dataset - returns 363 (on Phone,Plast, Clast)
distinctRXredempfams<-as_tibble(RXredemps [!duplicated(RXredemps[c(4,6,54)]),])
...
#create total unique fams - returns 12,591 (by Phone, Plast, Clast)
fulluniquefams<-as_tibble(fullmerge[!duplicated(fullmerge[c(4,6,54)]),])
