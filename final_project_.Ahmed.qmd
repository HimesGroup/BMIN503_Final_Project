...
The study was conducted in hospitals located in the Oromia region, the Amhara region, and the capital, Addis Ababa.
...
The median gestational age at delivery was 34, 34, and 33 weeks in Addis Ababa, Amhara region, and Oromia region respectively. BirthWeight also differ between the three region, Oromia has the lowest birthweight ( median=1730 grams).
...
fig <- ggplot(merged_data, aes(x = region, y =birthweight, fill = region)) +
  ...
  labs(title = "Birth Weight (g) by Region",
       x = "Region",
       y = "Birth Weight (g)") +
  theme_minimal()
...
fig.2 <-ggplot(merged_data, aes(x = region, y =gestageweeksatdelivery, fill = region)) +
  ...
  stat_summary(fun = median, geom = "text", aes(label = paste("Median =", round(..y.., 2))),
               vjust = -0.5, position = position_dodge(width = 0.75), size = 3) +
  labs(title = "Gestational week at Delivery  by Region",
       x = "Region",
...
ggsave("Ex.tiff", plot = fig)
plot_grid(fig, fig.2, fig.3, fig.4,fig.5,labels = "AUTO")
```
...
glm_fit_confounding <-glm(mortality ~ region + maternalage + firstpreg + birthweight + gestageweeksatdelivery + maritalstatus + occupation, data = merged_data, family =binomial)
...
model_500_to_2000  <- glm(mortality ~ region + maternalage + gestageweeksatdelivery + firstpreg + maritalstatus + occupation, data = subset_1500_to_2000   , family = binomial)
summary(model_500_to_2000  )
