...
ggmap(map2) + geom_density_2d(data = pm, mapping =  aes(x = lon, y = lat, color = PM2.5))+geom_point(data = pm, mapping =  aes(x = lon, y = lat, color = PM2.5), size = 2)

ggmap(map2)+geom_raster(aes(fill=pm$PM2.5))

pmt <- pm[c(1:1000),]

ggplot(data = pmt, aes(x = lon, y = lat)) + geom_raster(aes(fill=PM2.5))

ggplot(data = pm,aes(x=lon,y=lat, colour = PM2.5))+
   stat_density2d(data = pm, aes(fill=..level..), geom="polygon")
pmden <- pm %>% select(lat,lon,PM2.5)


ggplot()+geom_density_2d(data = pm, mapping =  aes(x = lon, y = lat, color = PM2.5), size = 1)


pmden$PM2.5 <- 1


ggmap(map2) +geom_density2d(data = pmden, aes(x = lon, y = lat)) +  stat_density2d(data = pmden, aes(x = lon, y = lat, colour = PM2.5, alpha =0.5),size = 0.01, bins = 16, geom = 'polygon') + scale_fill_gradient(low = "green", high = "red")
```
...
ggplot(pm.time)+geom_histogram(aes(x=Time))
...
```{r}
ggplot(pm.time)+geom_point(aes(x=Time, y = PM2.5))
...
ggplot(sound.time)+geom_point(aes(x=Time, y = S))
