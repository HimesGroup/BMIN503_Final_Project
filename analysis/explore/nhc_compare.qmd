...
```{r}
group_cols <- c("SSA_Code", "StateAbbrev", "StdCountyName", "collection_yr")

county_groups <- snf_level_data %>%
  group_by(across(allof(cols))) %>%
  summarize(
    cerfied_beds = sum(number_of_certified_beds, na.rm=TRUE),
    sum_avg_daily_residents = sum(avg_number_of_residents_per_day, na.rm=TRUE),
    mean_cna_reported_hprd = weighted.mean(reported_nurse_aide_staffing_hours_per_resident_per_day, avg_number_of_residents_per_day, na.rm=TRUE))
  )
```