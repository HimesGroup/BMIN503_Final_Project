...
```{r}
data$READMIT_72_HR <- as.factor(data$REVISIT_72_HR)
...
```{r}
data.fit <- data |>
  select(REVISIT_72_HR, READMIT_72_HR, Arrival, SEX, eth_cat, LANGIND, PAYOR_GROUP, 
        SVI_CATEGORY, ACUITY,COMPLEX_CHRONIC_CONDITION_IND, ED_DISPOSITION, Arrival, CAMPUS) |>
  drop_na() |>
  ...
  mutate(COMPLEX_CHRONIC_CONDITION_IND = factor(COMPLEX_CHRONIC_CONDITION_IND, levels = c(0,1),
...
```{r}
m1 <- glm(REVISIT_72_HR ~ Arrival + SEX + eth_cat + LANGIND + PAYOR_GROUP + ACUITY+ COMPLEX_CHRONIC_CONDITION_IND + Arrival + ED_DISPOSITION + CAMPUS, data = data.fit,family = binomial())
```
