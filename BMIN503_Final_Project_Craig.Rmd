...
library(tableone)
#install.packages("lubridate") #date time package
library(lubridate)
```
## Overview
...
data_all_na <- read.csv("https://raw.githubusercontent.com/scraig10/BMIN503_Final_Project/master/data_all_11_28_2018.csv", header=T, na.strings=c(""," ","NA", "-")) #assign NAs to blanks, spaces, and dashes
...
```{r}
head(data_all_na)
data_all_na$arr_date <- as.Date(data_all_na$ED.Arrive, format= "%m/%d/%Y")
summary(data_all_na$arr_date)
hist(data_all_na$arr_date)

#***trying to split time out into separate column
#data_all_na$arr_time <- format(as.POSIXct(data_all_na$ED.Arrive) ,format = "%H:%M:%S") 
#data_all_na$arr_time <- strptime(data_all_na$ED.Arrive, "%H:%M:%S")
#data_all_na$arr_time <- format(data_all_na$ED.Arrive,"%H:%M:%S")

```

#Table 1. Categorical variables
```{r}
#Insert table of race, sex, total patients
# N unique patients
#describe(X6f3fee63188d4821a7f220d87676b0c5): Distinct MRN's 2626

#X6f3fee63188d4821a7f220d87676b0c5 %>%
#  group_by(MRN) %>%
#  count %>%
#  return

#dup_pt <- X6f3fee63188d4821a7f220d87676b0c5 %>% Make a column of just duplicated mrn's, find the range and average counts for each pt who was seen more than once
