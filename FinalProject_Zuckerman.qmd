...
Because substance use is not our outcome of interest, we will combine all substance use items into one binary variable called `substance.use`. Those who have used any substance will be assigned 1 while those who have not will be assigned 2. The substances included are: cigarettes, electronic vapor products, chewing tobacco, snuff, dip, snus, dissolvable tobacco products, cigars, cigarillos, little cigars, alcohol, marijuana, synthetic marijuana, prescription painkillers, cocaine, inhalants, heroin, methamphetamines, ecstasy, and illicit intravenous drugs. We will use the dichotomized versions of these variables in which a 1 indicates use and a 2 indicates no use.

```{r}

yrbs.female$substance.use <- ifelse(
  yrbs.female$qn30==1 | yrbs.female$qn34==1 | yrbs.female$qn37==1 | yrbs.female$qn38==1 | yrbs.female$qn40==1 | yrbs.female$qn45==1 | yrbs.female$qn48==1 | yrbs.female$qn49==1 | yrbs.female$qn50==1 | yrbs.female$qn51==1 | yrbs.female$qn52==1 | yrbs.female$qn53==1 | yrbs.female$qn54==1 | yrbs.female$qn55==1, 1, 2)

table(yrbs.female$substance.use, useNA="ifany")

# We are able to confirm that there are 1810 rows in which all dichotomous drug variables have a value of 2.
nrow(yrbs.female[(yrbs.female$qn30==2 & !is.na(yrbs.female$qn30)) & 
                  (yrbs.female$qn34==2 & !is.na(yrbs.female$qn34)) & 
                  (yrbs.female$qn37==2 & !is.na(yrbs.female$qn37)) & 
                  (yrbs.female$qn38==2 & !is.na(yrbs.female$qn38)) & 
                  (yrbs.female$qn40==2 & !is.na(yrbs.female$qn40)) & 
                  (yrbs.female$qn45==2 & !is.na(yrbs.female$qn45)) &
                  (yrbs.female$qn48==2 & !is.na(yrbs.female$qn48)) & 
                  (yrbs.female$qn49==2 & !is.na(yrbs.female$qn49)) &
                  (yrbs.female$qn50==2 & !is.na(yrbs.female$qn50)) & 
                  (yrbs.female$qn51==2 & !is.na(yrbs.female$qn51)) & 
                  (yrbs.female$qn52==2 & !is.na(yrbs.female$qn52)) & 
                  (yrbs.female$qn53==2 & !is.na(yrbs.female$qn53)) & 
                  (yrbs.female$qn54==2 & !is.na(yrbs.female$qn54)) & 
                  (yrbs.female$qn55==2 & !is.na(yrbs.female$qn55)),])

# For subjects who did not report use of any drug (i.e., no 1s) but have missing values (i.e., NAs) for some drugs, they will have a missing value for substance.use. This is because we cannot know whether they used the drugs for which a missing value is present. We can see how many participants this is true for by counting the number of rows for which: (1) they do not have a 1 for any dichotomous drug use variables, and (2) they are missing at least one dichotomous drug use variable. When we run this code, we confirm that the number of rows matches the number of NAs in our substance.use variable.
nrow(yrbs.female[
                (yrbs.female$qn30!=1 & yrbs.female$qn34!=1 & yrbs.female$qn37!=1 & yrbs.female$qn38!=1 & yrbs.female$qn40!=1 & yrbs.female$qn45!=1 & yrbs.female$qn48!=1 & yrbs.female$qn49!=1 & yrbs.female$qn50!=1 & yrbs.female$qn51!=1 & yrbs.female$qn52!=1 & yrbs.female$qn53!=1 & yrbs.female$qn54!=1 & yrbs.female$qn55!=1) &
                (is.na(yrbs.female$qn30) | is.na(yrbs.female$qn34) | is.na(yrbs.female$qn37) | is.na(yrbs.female$qn38) | is.na(yrbs.female$qn40) | is.na(yrbs.female$qn45) | is.na(yrbs.female$qn48) | is.na(yrbs.female$qn49) | is.na(yrbs.female$qn50) | is.na(yrbs.female$qn51) | is.na(yrbs.female$qn52) | is.na(yrbs.female$qn53) | is.na(yrbs.female$qn54) | is.na(yrbs.female$qn55)),
])
```
...
# substance.use - use of any substance
yrbs.female$substance.use <- factor(yrbs.female$substance.use, levels=1:2, labels=c("Yes","No"))
...
table.1 <- tbl_svysummary(data=yrbsdes, by="sex.orientation", 
               ...
               type = list(qn57 ~ "categorical", qn62 ~ "categorical", substance.use ~ "categorical"),
               sort = list(grade ~ "frequency", qn59 ~ "frequency", qn60 ~ "frequency"),
               include=c(grade, race4, qn57, qn58, q64, qn59, qn60, qn62, qn63, q63, substance.use, qn82, qn83),
              label=list(grade~"Grade", 
                         ...
                         q63~"Type of pregnancy prevention method used during previous sexual encounter with an opposite-sex partner",
                         qn82~"Ever tested for HIV/AIDS",
                         qn83~"STD test in past 12 months",
                         substance.use~"Ever used any substance/drug")) %>%
  add_overall() 
```
...
plot1.all <- ggbarweight_svy(plot1.all.des, q63) + 
  ylab("Proportion") +
...
plot1.het <- ggbarweight_svy(plot1.heterosexual.des, q63) + 
  ylab("Proportion") +
  xlab("Pregnancy Prevention Method") + 
...
plot1.sm <- ggbarweight_svy(plot1.sexual.minority.des, q63) + 
  ylab("Proportion") +
  xlab("Pregnancy Prevention Method") + 
...
#### Adjusting for the use of any substance

```{r}
#code will go here
```

#### Adjusting only for sex-related behaviors: age at first time having sex, sex of sexual partners, \# of lifetime sexual partners, \# of sexual partners in past 3 months, condom use during previous sexual encounter
...
### Figure 1 - Pregnancy Prevention Methods by Sexual Orientation
...
> Code + summary of findings will go here
