...
NH_clean <-IST_nh %>% #Create a new data set with outcome variables and only pertinent data
  ...
  dplyr::select(-(HOSPNUM:RDELAY), -(RATRIAL:RASP3), -(RDEF1:RDEF8), -(RDATE:DALIVE), -(DPLACE:NCB14)) %>% #removing columns not pertinent
  rename(HospLOS=DALIVED, gender=SEX) %>%
  drop_na(alive6m) %>% #remove subjects that don't have survival outcome
  mutate(VeryAdvAge = ifelse(AGE >=85, "yes", "no"))
...
ggplot(data=NH_alive6m, aes(x=gender, fill=alive6m)) +
  geom_bar(position="fill") +
  labs(title="Gender and 6 Month survival")

#Box plot of gender and Final Living Place
ggplot(data=NH_FLP, aes(x=gender, fill=FLP)) +
  geom_bar(position="fill")

#Box plot of gender and Final Functional Outcome
ggplot(data=NH_FFO, aes(x=gender, fill=FFO)) +
  geom_bar(position="fill")
...
alive6m.top.glm <-glm(alive6m ~ AGE + HospLOS + RSBP + STYPE + RCONSC + DefDysphagia + SEX, data= NH_alive6m, family = binomial(logit))
...
alive6m.top.rf <- randomForest(alive6m ~ AGE + HospLOS + RSBP + STYPE + RCONSC + DefDysphagia + SEX, data=NH_alive6m, ntree=100, importance=TRUE)
alive6m.top.rf
...
for(i in 1:K){
   ...
   #GLM train/test
    glm <- glm(alive6m ~ AGE + HospLOS + RSBP + STYPE + RCONSC + DefDysphagia + SEX, data=train, family=binomial(logit))
    glm.pred.curr <- predict(glm, test, type="response")
    alive6m.top.pred_outputs.glm[1:length(s[s==i]) + offset] <- glm.pred.curr
    
    #RF train/test
    rf <- randomForest(alive6m ~ AGE + HospLOS + RSBP + STYPE + RCONSC + DefDysphagia + SEX, data=train, ntree=100)
    rf.pred.curr <- predict(rf, newdata=test, type="prob") 
...
FLP.top.glm <-glm(FLP ~ AGE + HospLOS + RSBP + STYPE + RCONSC + DefDysphagia + SEX, data= NH_FLP, family = binomial(logit))
...
FLP.top.rf <- randomForest(FLP ~ AGE + HospLOS + RSBP + STYPE + RCONSC + DefDysphagia + SEX, data=NH_FLP, ntree=100, importance=TRUE)
FLP.top.rf
...
for(i in 1:K){
    ...
    #GLM train/test
    glm <- glm(FLP ~ AGE + HospLOS + RSBP + STYPE + RCONSC + DefDysphagia + SEX, data=train, family=binomial(logit))
    glm.pred.curr <- predict(glm, test, type="response")
    FLP.top.pred_outputs.glm[1:length(s[s==i]) + offset] <- glm.pred.curr
    
    #RF train/test
    rf <- randomForest(FLP ~ AGE + HospLOS + RSBP + STYPE + RCONSC + DefDysphagia + SEX, data=train, ntree=100)
    rf.pred.curr <- predict(rf, newdata=test, type="prob") 
...
FFO.top.glm <-glm(FFO ~ AGE + HospLOS + RSBP + STYPE + RCONSC + DefDysphagia + SEX, data= NH_FFO, family = binomial(logit))
...
FFO.top.rf <- randomForest(FFO ~ AGE + HospLOS + RSBP + STYPE + RCONSC + DefDysphagia + SEX, data=NH_FFO, ntree=100, importance=TRUE)
FFO.top.rf
...
for(i in 1:K){
    ...
    #GLM train/test
    glm <- glm(FFO ~ AGE + HospLOS + RSBP + STYPE + RCONSC + DefDysphagia + SEX, data=train, family=binomial(logit))
    glm.pred.curr <- predict(glm, test, type="response")
    FFO.top.pred_outputs.glm[1:length(s[s==i]) + offset] <- glm.pred.curr
    
    #RF train/test
    rf <- randomForest(FFO ~ AGE + HospLOS + RSBP + STYPE + RCONSC + DefDysphagia + SEX, data=train, ntree=100)
    rf.pred.curr <- predict(rf, newdata=test, type="prob") 
...
#AUC 
auc(NH_alive6m$alive6m, alive6m.top.glm.pred) #AUC = 0.7183
...
auc(obs_outputs, pred_outputs.rf) #AUC = 0.6594
