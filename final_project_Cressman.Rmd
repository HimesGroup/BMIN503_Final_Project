...
County-level time-series case data can be accessed through the CDC's COVID Data Tracker website. The CDC reports the 7-day rolling average of new cases per 100,000 people based on 2019 U.S. Census estimates (8). 
...
# time-dependent variant proportion estimates
wgs <- read_csv("wgs_variant_proportion_estimates.csv")
...
full_df <- next_case_fi %>% mutate(county_pop = population) %>%
  ...
  mutate(vacc_incidence_per100thou = vacc_incidence_n / county_pop * 100000, # adjust by county population
         ...
         cdc_lineage_simple = as.factor(cdc_lineage_simple)) %>% 
  select(county, year_week, case_mean, case_mean_per100thou, next_week_mean,
         ...
         vacc_prevalence_per100thou, cdc_lineage_simple, mean_retail_rec, mean_grocery_pharm, mean_parks, 
...
When examining the vaccine incidence variable, we see the results we would expect: mean vaccine incidence densities are 122.77 and 74 in the "no increase"  and "increase" groups, respectively.
...
ggplot(data = covid, aes(x = next_week_increase, y = mean_per_pop_fully_vacc)) +
  ...
  ylab("Mean Percent of Population Fully Vaccinated") +
...
ggplot(data = covid, aes(x = next_week_increase, y = vacc_prevalence_per100thou)) +
  ...
  theme(
    ...
    plot.title = element_text(hjust = 0.5, size = 12)) +
  xlab("Outcome: Next Week's SARS-CoV-2 Cases") +
  ylab("Mean Vaccine Prevalence Density") +
  labs(title = "Vaccination by Outcome Group") 
...
ggplot(data = covid, aes(x = next_week_increase, y = vacc_incidence_per100thou)) +
  ...
  theme(
    ...
    plot.title = element_text(hjust = 0.5, size = 12)) +
  xlab("Outcome: Next Week's SARS-CoV-2 Cases") +
  ylab("Mean Vaccine Incidence Density") +
  labs(title = "Vaccination by Outcome Group") 
...
We note strong correlations (both negative and positive) between some pairs of variables. Additional tests will be used to check for multicollinearity in the subsequent regression models.
...
The above table shows that two of the independent variables are significantly associated with case increase: mean vaccine incidence density and the social mobility variable for retail and recreation.

Based on the above univariate analysis, a multivariate model is fit using the standardized form of mean vaccine incidence density and retail and recreation as independent variables. Table output is generated using gtsummary::tbl_regression. The model is checked for multicollinearity, which is not an issue.
...
# fit model
mod_vacc <- glm(next_week_increase ~ cdc_lineage_simple + vacc_inc_per100thou_st + mean_retail_rec,
...
## Main Model
Both independent variables identified as significant in the univariate modeling stage are significant at the <0.001 level when included in the multivariate logistic regression model. When mean retail and recreation was used as a single explanatory variable predicting an increase in case incidence density, it was significant at the <0.05 level. Further, when used in combination with vaccine incidence density, the p-value of mean retail and recreation decreases. This indicates that controlling for vaccine incidence density shrinks the variance of mean retail and recreation. Therefore, we can be more confident about the association between mean retail and recreation and case increase when controlling for vaccine incidence density.

The odds ratio for vaccine incidence density is 0.62 (95% CI: 0.51 - 0.73). This means that when controlling for mean retail and recreation, for each additional 1 vaccine per 100,000 people in a given week, the likelihood of an increase in mean case incidence density in the following week is 38% lower.

The odds ratio for mean retail and recreation is 1.02 (95% CI: 1.01 - 1.03). When controlling for mean vaccine incidence density, there is a 2% increase in odds of cases increasing when retail and recreation activity increases by 1 percentage point.
...
As a final step, the areas  under the curve (AUC) for training and test sets are calculated, and the corresponding receiver operating characteristic (ROC) curves are generated. The AUCs are 0.63 (95% CI: 0.59 - 0.66) and 0.62 (95% CI: 0.58 - 0.66) for training and test data, respectively. These numbers indicate that the predictive accuracy is not very high, and therefore, the model does not distinguish well between no case increase and case increase. However, Joe Romano commented that because the AUC is better than random guessing, it means the model is learning something and is therefore of potential value.
