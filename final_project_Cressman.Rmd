...
# first_delta = starts when the first delta variant is identified by study: 2021-15 (week ending April 17, 2021)
# dominant_delta = starts when delta makes up >=50% of samples in study: 2021-25 (week ending June 26, 2021)
...
#vacc_plot + transition_reveal(covid_time$yearweek_num) 
#anim_save("vacc_plot.gif", vacc_plot + transition_reveal(covid_time$yearweek_num))

animate_vacc <- vacc_plot + transition_reveal(covid_time$yearweek_num) 
animate(animate_vacc, height = 350, width = 600)
#anim_save("vacc_plot3.gif")
```
...
#anim_save("case_plot3.gif")

#anim_save("case_plot.gif", case_plot + transition_reveal(covid_time$yearweek_num))


vacc_plot / case_plot + plot_layout(guides = "collect")  
# plot_comb + transition_reveal(covid_time$yearweek_num) 
...
It should also be considered that a large increase in cases may predict vaccination incidence, rather than vaccine incidence predicting case incidence.
...
glm_output <- covid_clean %>%
  ...
  mutate(Variable = recode_factor(Variable,
                                  ...
                                  "vacc_inc_per100thou_st" = "Mean vaccine incidence density (st.)",
                                  "mean_retail_rec" = "Google: mean retail and recreation",
                                  "vacc_prev_per100thou_st" = "Mean vaccine prevalence density (st.)",
...
The odds ratio for vaccine incidence density is 0.67 (95% CI: 0.56 - 0.80). This means that when controlling for the delta-dominant period, for each additional 1 vaccine per 100,000 people in a given week, the likelihood of an increase in case incidence density in the following week is 33% lower.
...
```
## Future Analyses
Additional regression models predicting vaccine incidence density based on cased incidence density should be fit.
