...
Each of these datasets contains county-level time-series data. The data were aggregated by county and MMWR week. As the Google Community Mobility Reports date back to February 15, 2020, and the other datasets did not start that early, rows of zeros were added so that the vaccine, case, and social mobility datasets could be merged.
...
for (county in counties) {
  ...
  for (i in 1:nrow(county_data) - 1) {
    county_data$new_vaccinations[i] = county_data$fully_vaccinated[i] -                                                         county_data$fully_vaccinated[i + 1]                           }                                                                                             county_new <- rbind(county_new, county_data)
}
