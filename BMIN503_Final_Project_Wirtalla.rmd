...
* __National Board of Economic Research (NBER)'s National Plan and Provider Enumeration System (NPPES) [website](https://www.nber.org/research/data/national-plan-and-provider-enumeration-system-nppes) for Physician NPI Codes:  
...
The distributions for Total Population and Median Income look better after log transformation so they were used in the statistical modeling. The Surgeons per 100K Population variable shows as bimodal so I decided to keep it in its raw value.  


#### Statistical Analysis
Multivariable linear regression was conducted using the following formula:

$$\Large RPct_{ij}=f(SU_{i,j-1},\ PVS_{j-4},\ SW_{i,j-1},\ GDP_{ij},\ Pov_i,\ CR_{ij},\ \eta_j,\ FIPS_{i})$$  
...
Not surprisingly, due to the criteria for reliability, the Unreliable estimates appear to be mostly from smaller, more rural counties with higher Republican voting shares. Interestingly, 71% of the unreliable counties were from the 2016 cohort.    
  
#### County Maps  

In order to better visualize any potential relationships between surgery utilization and voting results, I generated choropleth graphs by US County for each year for Surgical Uilization Rates and Republican Voting %. While there are obvious differences between the graphs, you may note that the Midwest portion of the United States displays the largest collection of high surgical ulilization counties, a pattern even more proncounced in correponding election maps. This suggests a slight lean towards higher surgical rates in Republican counties.  
...
models[['2.Multivariable: Prior Vote Share Excluded (for R-Sqr)']] <-lm(data=analytical_cohort,
                                                                   r_pct ~ adj_surg_rate + 
                                                                     surgeons_per1kpop + 
                                                                     pct_gdp_chg + 
                                                                     pct_fairpoor + 
...
models[['3.Multivariable: All Surgeries']]                     <-lm(data=analytical_cohort, 
                                                                  r_pct ~ adj_surg_rate + 
                                                                    ...
                                                                    PovertyAllAgesPct + 
                                                                    merge_year)
models[['4.Multivariable w/State Fixed Effects']]               <-lm(data=analytical_cohort, 
                                                                   r_pct ~ adj_surg_rate + 
                                                                     past_vote_share + 
                                                                     surgeons_per1kpop + 
                                                                     pct_gdp_chg + 
                                                                     ...
                                                                     factor(merge_year) + 
                                                                     factor(state))
models[['5.Sensitivity Model: Multivariable']]                  <-lm(data=analytical_cohort, 
                                                                   r_pct ~ adj_surg_rate + 
                                                                     past_vote_share + 
                                                                     surgeons_per1kpop + 
                                                                     pct_gdp_chg + 
                                                                     ...
                                                                     PovertyAllAgesPct + 
                                                                     merge_year)
models[['6.Sensitivity Model: w/State Fixed Effects']]          <-lm(data=analytical_cohort, 
                                                                   r_pct ~ adj_surg_rate + 
                                                                     past_vote_share + 
                                                                     surgeons_per1kpop + 
                                                                     pct_gdp_chg + 
...
analytical_cohort_noneg<-analytical_cohort %>%
  filter(surg_neg==0)
models[['7.Neg Values Excluded']]                               <-lm(data=analytical_cohort_noneg, 
                                                                   r_pct ~ adj_surg_rate + 
                                                                     past_vote_share + 
                                                                     surgeons_per1kpop + 
                                                                     pct_gdp_chg + 
                                                                     ...
                                                                     PovertyAllAgesPct + 
                                                                     factor(merge_year))
models[['8.Neg Values Excluded + State Fixed Effects']]         <-lm(data=analytical_cohort_noneg, 
                                                                   r_pct ~ adj_surg_rate + 
                                                                     past_vote_share + 
                                                                     surgeons_per1kpop + 
                                                                     pct_gdp_chg + 
...
Univariate results showed a significant association between surgical utilization and Republican voting % (Model 1; $\beta$=0.167, p<0.001). In order to check the R^2^ prior to the addition of the prior vote share covariate (known to be the biggest predictor of current election vote share), Model 2 showed an R^2^ of 0.499. The primary mulivariable model (Model 3) indicated that, after adjustment for prior vote share, surgical workforce, economic, and socioeconomic factors, every additional surgical discharge per 1,000 Medicare enrollees is associated with a 0.046% increase in Republican vote share in the following year's election ($\beta$=0.046; p<0.001). This association did not remain after FIPS counties were added as individual fixed effects (Model 4; $\beta$=0.000, p=0.999), suggesting that the significant results in the Multivariable model are likely due to some (as-yet) unobserved factors at the county level. Sensitivity analyses were conducted where % Fair/Poor Health and % H.S Graduates were excluded from the models due to significant missingness. The relationship between surgery utilization and Republican % remained significant (Model 5; $\beta$=0.036, p<0.001) in the model without FIPS fixed effects and, as before, was not significant with FIPS fixed effects included (Model 6). Finally, a further sensitivity analysis was performed for those counties with "unreliable" values, as indicated in the Dartmouth Atlas data by negative values. Interestingly, the association between surgery utilization and election results was not significant after these counties were removed (Model 7; $\beta$=0.002, p=0.812).  
  
In order to test the secondary hypotheses, subset analyses was performed for TURP, Hip Fracture, Back, and CABG surgeries:
...
models2[['Multivariable: TURP Surgery']]                        <-lm(data=analytical_cohort, 
                                                                   r_pct ~ adj_turp_surg_rate + 
                                                                     ...
                                                                     PovertyAllAgesPct + 
                                                                     factor(merge_year))
models2[['Multivariable: Back Surgery']]                        <-lm(data=analytical_cohort, 
                                                                     r_pct ~ adj_back_surg_rate + 
                                                                       ...
                                                                       PovertyAllAgesPct + 
                                                                       factor(merge_year))
models2[['Multivariable: Hip Fracture Surgery']]                <-lm(data=analytical_cohort, 
                                                                     r_pct ~ adj_hipfx_surg_rate + 
                                                                       ...
                                                                       PovertyAllAgesPct + 
                                                                       factor(merge_year))
models2[['Multivariable: CABG Surgery']]                        <-lm(data=analytical_cohort, 
                                                                     r_pct ~ adj_cabg_surg_rate + 
                                                                       ...
                                                                       PovertyAllAgesPct + 
                                                                       factor(merge_year))
models2[['Sensitivity: TURP Surgery, Neg Excluded']]            <-lm(data=analytical_cohort_turpnoneg, 
                                                                     r_pct ~ adj_turp_surg_rate + 
                                                                       past_vote_share + 
                                                                       surgeons_per1kpop + 
                                                                       pct_gdp_chg + 
                                                                       ...
                                                                       PovertyAllAgesPct + 
                                                                       factor(merge_year))
models2[['Sensitivity: Back Surgery, Neg Excluded']]            <-lm(data=analytical_cohort_backnoneg, 
                                                                    r_pct ~ adj_back_surg_rate + 
                                                                      past_vote_share + 
                                                                      surgeons_per1kpop + 
                                                                      pct_gdp_chg + 
                                                                      ...
                                                                      PovertyAllAgesPct + 
                                                                      factor(merge_year))
models2[['Sensitivity: Hip Fracture Surgery, Neg Excluded']]    <-lm(data=analytical_cohort_hipfxnoneg, 
                                                                     r_pct ~ adj_hipfx_surg_rate + 
                                                                       past_vote_share + 
                                                                       surgeons_per1kpop + 
                                                                       pct_gdp_chg + 
                                                                       ...
                                                                       PovertyAllAgesPct + 
                                                                       factor(merge_year))
models2[['Sensitivity: CABG Surgery, Neg Excluded']]            <-lm(data=analytical_cohort_cabgnoneg, 
                                                                     r_pct ~ adj_cabg_surg_rate + 
                                                                       past_vote_share + 
                                                                       surgeons_per1kpop + 
                                                                       pct_gdp_chg + 
...
These results suggest that CABG utilization rates are the largest predictor of Republican voting %, while hip fractures, back, and TURP procedure utilization does not show an association with election results. This is in contrast to my sub-hypotheses 1b and 1d that more "elective" surgeries with less variation, like back and TURP procedures, would demonstrate a positive relationship with Republican voting share. Hip fractures were the only procedure that adhered to my subset hypotheses, however it should be noted that they were nearest to achieving significance ($\beta$=0.054, p=0.102) after CABG. If significant, the positive $\beta$ would have joined CABG in being at odds with my original hypothesis. Sensitivity analyses excluding counties with "unreliable" results did not yield significantly different results from the original models. 

Overall, the linear regression results were inconsistent across the various models, most notably for the overall models the excluded unreliable county values and also the procedure-specific results that mostly were in contrast to my secondary hypotheses. It's unclear if this is due to lack of appropriate adjustment variables, observations, data lags, or other limitations. Further investigation is needed in order to determine if the occasional significant results seen can offer a consistent insight into whether variation in surgical decision-making is reflected in election results.  
