...
ac2 <- analytical_cohort %>% dplyr::select(adj_surg_rate , r_pct , past_vote_share , pct_gdp_chg , pct_fairpoor , pct_hsgrad , pct_unemp , pct_nhblack , pct_rural , pct_uninsured , tot_pop , med_income , PovertyAllAgesPct, merge_year)
...
ac2 %>%
  tbl_summary(
    ...
    missing="no"
  ) %>% add_overall() %>% modify_spanning_header((c("stat_1", "stat_2") ~ "**Year**")) %>% as_gt() %>% gt::tab_header("Table 1. County-Level Characteristics by Year") %>% opt_align_table_header(align = "left")
