...
This is an interdisciplinary problem that draws knowledge from data science, clinical microbiology, statistics, and data visualization. I have been working with Dr. Irv Nachamkin, Director of the Divison of Laboratory Medicine at HUP. He has helped direct me to relevant analyses based on his domain knowledge of clinical microbiology. I have also met with Dr. Rebecca Hubbard, Associate Professor of Biostatistics. She gave me useful advice on conducting linear regressions on my data and visualizing the data.
...
fig_freqDist <- ggplot(by_ID, aes(x=count)) +
...
ggsave(file="Figures/FY15_freqDist.pdf",plot=fig_freqDist,dpi=300)

by_ID20 <- by_ID %>%
  filter(count>=20)

fig_freqDist20 <- ggplot(by_ID20, aes(x=count)) +
  geom_histogram(binwidth=1) +
  theme_bw() + 
  scale_y_continuous(expand=c(0,0)) +
  scale_x_continuous(expand=c(0,0)) +
...
ggsave(file="Figures/FY15_freqDist20.pdf",plot=fig_freqDist20,dpi=300)
...
fig_freqDistLoc <- ggplot(by_loc, aes(x=count)) +
...
ggsave(file="Figures/FY15_freqDistLoc.pdf",plot=fig_freqDistLoc,dpi=300)
...
fig_freqDist30d <- ggplot(bc_30d_ID, aes(x=count)) +
  geom_histogram(binwidth=1) +
  theme_bw() + 
  scale_y_continuous(expand=c(0,0)) +
  scale_x_continuous(expand=c(0,0)) +
...
fig_cultures <- ggplot(by_isolate, aes(reorder(Final_Report,-count),count)) +
...
ggsave(file="Figures/FY15_cultures.pdf",plot=fig_cultures,dpi=300)
...
fig_highut <- ggplot(df,aes(Julian,Study_ID)) +
  geom_point() +
  scale_x_continuous(breaks=c(0,50,100,150,200,250,300,350,400))

ggsave(file="Figures/FY15_highut.pdf",plot=fig_highut,dpi=300)
...
#was able to use split30days in my bloodculturesproject R project. copied and pasted and then got an error here.

cultures.pos.loc_subsets <- split(cultures.pos.loc,cultures.pos.loc$Study_ID)
...
#why does it still say I have 11737 levels in Study_ID when there should only be 3??
test <- cultures.pos.loc[cultures.pos.loc$Study_ID %in% c(7,14,4584),]

#getting weird result...
...
for (i in 1:nrow(culturesbyLoc_30d)){
  ...
  y <- x[x$Julian>date,]
  g <- x[x$Julian==date,"group"][1] #there are multiple entries if it's polymicrobial but the group should be the same
  z <- y[y$group==g,]
