...
culturesbyLoc <- cultures.pos %>%
  group_by(Study_ID,Julian,Location,Final_Report) %>%
  summarise(count=n())

culturesbyLoc_subsets <- split(culturesbyLoc,culturesbyLoc$Study_ID)

#use split30days to group sets taken within 30d of each other
#culturesbyLoc_30d <- lapply(culturesbyLoc_subsets,splitDates)
culturesbyLoc_30d <- lapply(culturesbyLoc_subsets,split30days)
culturesbyLoc_30d <- as.data.frame(do.call("rbind",culturesbyLoc_30d))
culturesbyLoc_30d %<>%
  arrange(Study_ID,Julian)
