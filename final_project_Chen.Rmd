...
#check that by_isolate is the right dataframe since I used that name for two things.
#has more rows than bc_30d_ID20_highuse because for some days, the different sets had different results
#use num_set.y which has the accurate set corresponding to the final_report for that day
highuse_isolate <- merge(bc_30d_ID20_highuse,by_isolate,by=c("Study_ID","Julian"))

highuse_isolate %<>%
  select(Study_ID,Julian,Location,group_30,Final_Report,num_set.y) %>%
  arrange(Study_ID,Julian)
colnames(highuse_isolate)[6] <- "num_set"
...
s <- sample(unique(highuse$Study_ID),50,replace=FALSE)
s <- droplevels(s)
...
#show which cultures were positive
pheatmap(mat,
         main = "Pattern of blood cultures in high-utilizers",
         cluster_rows = F, cluster_cols = F,
         cellwidth = 20, cellheight = 20, fontsize = 20,
         border_color = "grey", 
         color = colorRampPalette(c("white","black"))(150),
       ...
       display_numbers = matrix(ifelse(mat_FR > 0, "*", ""), nrow(mat_FR)),
       show_rownames = FALSE,
       filename="~/scripts/EPID600_Final_Project/Figures/heatmap_HU_pos.pdf",
       width=30,
       height=20)
