...
summary(numFollowUpCultures$num_set_postPosID) #mean 4.979, 3rd quartile 7
summary(numFollowUpCultures$num_days_postPosID) #mean 2.932, 3rd quartile 4
...
#make binary variable for num follow-up: 
...
ggplot(nLoc_top,aes(reorder(Location,-num_set_postPosID),num_set_postPosID)) +
...
ggplot(nLoc_top,aes(reorder(Location,-num_days_postPosID),num_days_postPosID)) +
  #geom_jitter(width=0.1) +
  #geom_violin() +
  geom_boxplot() +
  theme_bw() +
...
#same
nPathLoc_top <- numFollowUpCultures %>%
  filter(Final_Report %in% topPathNames) %>%
  filter(Location %in% topLocNames)
nPathLoc_top <- droplevels(nPathLoc_top)
...
sig_pvalues
#aboveAverage, highSets, num_set_postPosID, count are significant
#highDays is highly correlated with aboveAverage, highSets, and num_set_postPosID
...
#546 patients with more than 13 bc taken.
nrow(ID_highuse)
...
#number of followup cultures is greater for high utilizers by 3-4 sets
...
summary(hu_followup_30d$num_set_postPosID)

#number of followup cultures is greater for high utilizers by about 10 sets
t.test(hu_followup_30d$num_set_postPosID,numFollowUpCultures$num_set_postPosID,alternative="two.sided")
plot(density(hu_followup_30d$num_set_postPosID))
plot(density(numFollowUpCultures$num_set_postPosID))
...
plot(density(hu_followup_30d$num_days_postPosID))
plot(density(numFollowUpCultures$num_days_postPosID))
