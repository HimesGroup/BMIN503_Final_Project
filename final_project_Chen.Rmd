...
#make binary variable for num follow-up greater than average (5): 
mean(numFollowUpCultures$num_set_postPosID)
numFollowUpCultures %<>%
  mutate(aboveAverage = factor(ifelse(num_set_postPosID<5,0,1),levels=c(0,1),labels=c("low","high")))
...
nLoc_10 <- numFollowUpCultures %>%
  filter(numFollowUpCultures$Location %in% nLoc_10$Location)
nLoc_10 <- droplevels(nLoc_10)

other <- numFollowUpCultures %>%
  filter(!(numFollowUpCultures$Location %in% nLoc_20$Location))
other <- droplevels(other)
#combine levels
levels(other$Location) <- rep("Other",nlevels(other$Location))

culturesLoc <- rbind(nLoc_10,other)

ggplot(culturesLoc,aes(Location,num_set_postPosID)) +
  #geom_jitter() +
  #geom_violin() +
  geom_boxplot() +
  theme_bw() +
  ...
  labs(x="Location", 
       y="Number of follow-up cultures",
       title="Distribution of follow-up cultures by location")

fit_loc <- glm(num_set_postPosID ~ Location,data=culturesLoc)
summary(fit_loc)

fit_loc_aov <- aov(num_set_postPosID ~ Location,data=culturesLoc)
summary(fit_loc_aov)
...
chisq.test(table(loc=culturesLoc$Location,followUp=culturesLoc$aboveAverage))

fit_loc <- glm(aboveAverage ~ Location,data=culturesLoc,family="binomial")
#RP3, RP6, RP7,S10, and Other are significant. doesn't look like it should be though.
summary(fit_loc)

ggplot(culturesLoc,aes(Location))+
  geom_bar(aes(fill=aboveAverage),position="dodge")
...
nPath_10 <- culturesLoc %>%
  filter(culturesLoc$Final_Report %in% nPath_10$Final_Report)
nPath_10 <- droplevels(nPath_10)
...
culturesLocPath <- rbind(nPath_10,other)


ggplot(culturesLocPath,aes(Final_Report)) +
  geom_bar(aes(fill=aboveAverage),position="dodge")

fit_path <- glm((aboveAverage=="low") ~ (Final_Report=="SA"),data=culturesLocPath,family="binomial")
summary(fit_path)
#EC is significant. I think in having lower follow-ups.

fit_path <- glm(num_set_postPosID ~ Final_Report,data=culturesLocPath)
summary(fit_path) #SA is significant

fit_path <- glm(num_days_postPosID ~ Final_Report,data=culturesLocPath)
summary(fit_path) #SA is even more significant

fit_path <- glm((aboveAverage=="high")~Final_Report,
                 data=culturesLoc,family="binomial")
summary(fit_path)

chisq.test(table(aboveAvg=culturesLocPath$aboveAverage,path=culturesLocPath$Final_Report))
```
...
test <- lapply(highuse_subset,function(df){
...
highuse <- lapply(test,normalizeDate)
