...
#use bc_30d_ID20. Has Study_ID's with more than 20 bc in a 30-day period.
...
for (i in 1:nrow(bc_30d_ID20)){
  ...
  x <- x[which(x[,"group_30"] %in% bc_30d_ID20[i,"group_30"]),]
  List[[i]] <- x
}
...
ggplot(numFollowUpCultures,aes(reorder(Final_Report,-num_set_postPosID),num_set_postPosID)) +
  ...
  labs(x="Isolate", 
       ...
       title="Number of follow-up cultures based on isolate")
