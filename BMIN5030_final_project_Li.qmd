...
Now we are ready to look at the influenza clade distribution in each state during influenza seasons. First, I will write a function that will graph a stacked line chart given the season, state, and influenza subtype (H1N1, H3N2, or influenza B). Because I don't have complete sequencing data for every week, I am graphing clade distribution by month. In addition, if there is missing data for a month, I would use the data from the previous month if available.
...
For all three influenza subtypes, we can see that clade distribution changes over time, usually with new clades replacing existing clades. There are more diversity with H1N1 and H3N2, whereas influenza B viruses are less diverse. Now let's focus on the four influenza seasons with the most sequencing data starting from 2015 and see if we can determine clade distribution patterns that are associated with duration of flu season. 

```{r}
#| label: graph_state_percent_positive 

# Graph percent positive test for state with longest duration of flu seaason
...
# Graph percent positive test for state with shortest duration of flu seaason
...
# Graph percent postive tests for states with the longest and shortest duration of flu season
...
The 2015-16 season had less . The clades that are present in each state are also drastically different, making it difficult to interpret if frequency or the actual difference in strains might be responsible. 
