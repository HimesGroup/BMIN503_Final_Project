...
df.1 <- data_all |>
  ...
  mutate(guide_conc_care = factor(guide_conc_care, levels = c("Yes", "No"),
                                  labels = c("Yes", "No"))) |>
  mutate(markers = factor(markers, levels = c("Yes", "No"),
                          labels = c("Yes", "No"))) |>
...
```{r}
#Descriptive Tables: Demographics

#Select demographic variables into a new data frame
df.demo <- df.1 |>                      
  select(sex, age, race, smoke, tx_site)
...
```{r}
#Descriptive Tables: Overview Summary 

df.test.char <- df.1 |>
  select(comprehensive_testing, markers, test_modality, guide_conc_care, alteration_di)
...
#Result comment: Patients who underwent plasma or plasma+tissue testing has comprehensive testing. Those who just had tissue alone did not? 

#Sig relationship b/w having markers tested prior to 1L and whether testing done on T/P/TP.
#Appears to be a relationship with receiving guideline concordant care and testing modality
```

```{r}
#TESTING - differences in comprehensive testing rates
...
tbl.comp.testing <- df.1 |>
  select(comprehensive_testing, test_modality, race, tx_site) |>
...
```{r}
#TESTING - TAT - Turn around time of results 
...
df.guidelines <- df.1 |>
  select(guide_conc_care, age, sex, race, smoke, tx_type, test_modality, comprehensive_testing, markers, alteration_di, alteration_tpb)
...
test.2 <- summary(glm(guide_conc_care ~ markers, data = df.guidelines, family = "binomial"))
test.2
...
glm.guidelines <- glm(guide_conc_care ~ sex + age + race + smoke + test_modality + markers + comprehensive_testing + alteration_di + alteration_tpb,
