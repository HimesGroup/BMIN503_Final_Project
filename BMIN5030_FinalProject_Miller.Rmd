...
# Methods
Changes in opioid overdose deaths by type (i.e., prescription opioid, heroin, or synthetic opioid) from 2015 to 2020 were characterized using [VSRR data](https://data.cdc.gov/NCHS/VSRR-Provisional-Drug-Overdose-Death-Counts/xkb8-kh2a/) on drug overdose deaths. Line plots of the number of individuals who died of an overdose by drug type in the US were generated to provide rationale for the key study questions. 

To evaluate changes in the dynamics of recent opioid prescribing, county-level opioid dispensing tables reported on the [CDC website](https://www.cdc.gov/drugoverdose/rxrate-maps/index.html) were transferred into Excel files by year. These files were merged with county-level geometry data by FIPS code to create choropleth maps by year. 
...
County-level data on opioid-specific overdose mortality are not publicly available, but county-level data on drug overdose over time is available from the [National Center for Health Statistics](https://data.cdc.gov/NCHS/NCHS-Drug-Poisoning-Mortality-by-County-United-Sta/pbkm-d27e). Despite the limitation of the lack of county-level data on opioid overdose deaths specifically, the preponderance of drug overdose deaths in the US are attributable to prescription or illicit opioids. For example, in 2020 there were 95,452 drug overdose deaths of which 71,487 (74.9%) were due to opioids. Therefore, county-level drug overdose deaths were used as a surrogate to evaluate the relationship between prescription opioid prescribing and social determinants of health on opioid overdose death in 2020. 

The relationships between these variables were characterized in univariable and multivariable linear regression models to evaluate the relative contribution of various factors in county-level drug overdose mortality rates. The specific county-level health indicators included the following variables in adults: binge drinking prevalence, depression prevalence, current smoking status, lack of health insurance, obesity prevalence, self-reported lack of leisure time, and self-reported poor recent physical health. To facilitate clinically relevant interpretations of regression coefficients, the opioid dispensing rate was scaled to reflect a 10-person per 100 resident change and prevalence variables were scaled to reflect a 5 percentage point change.

At various times throughout this project, I consulted with Dr. John Farrar, Professor of Epidemiology at the University of Pennsylvania, who is an expert in treatments for pain. My GitHub repository containing all information needed to reproduce these analyses as well as this HTML file can be found [here](https://github.com/cjm2718/BMIN503_Final_Project).
...
## create dataset to plot drug overdose deaths over time in the US
vsrr.us <- vsrr %>% 
...
# create truncated variable for death rate at 60 per 100,000 for mapping
...
places <- read_csv("PLACES__Local_Data_for_Better_Health__County_Data_2022_release.csv")
...
risk.data <- left_join(places.2020, county.disp.2020, by = "FIPS.code") %>% 
  ...
  mutate(log.opioid.rx = log(disp.rate.2020 + 0.1)) %>% 
...
The number of opioid overdose deaths (red line) more than doubled from approximately 30,000 per year in 2015 to 65,000 per year in 2020. In 2015, most of the approximately 30,000 opioid overdose deaths were due to natural and semi-synthetic opioids (blue line), which are prescription opioids such as oxycodone, morphine, and hydrocodone. Slightly fewer opioid overdose deaths in 2015 were due to heroin (purple line) and the smallest number were attributable to synthetic opioids (green line), which is predominated by illicit fentanyl and fentanyl analogs. After 2016 the number of opioid deaths increased dramatically, which was driven almost entirely by overdose deaths due to synthetic opioids. While prescription opioids may still serve as a gateway to the misuse and abuse of opioids, it remains unclear how opioid prescribing and dispensing practices are associated with drug overdose death rates in this new phase of the opioid epidemic.

```{r eval = TRUE, message = FALSE, warning = FALSE, fig.height=6, fig.width=12}
# line plot for opioid deaths overall and by type
...
ggplot(vsrr.us.plot, aes(x=Time, y=`Number of Deaths`, colour=`Opioid Type`)) +
  ...
  labs(title = "Opioid Overdose Deaths by Opioid Type (2015-2020)")

```


## Prescription Opioid Dispensing Rates
In 2015, prescription opioid dispensing rates exceeded 100 prescriptions per 100 residents in 855 (29.1%) of the 2,938 counties in the US reporting data. The counties with the highest rate of prescription opioid dispensing in 2015 were in Virginia, Kentucky, and Alabama. In the 10 counties with the highest rates of dispensing in 2015, more than 250 opioid prescriptions were dispensed per 100 residents, or 2.5 opioid prescriptions filled for every man, woman, and child in that year. 

```{r eval = TRUE, message = FALSE, warning = FALSE}
# number and percent of counties with dispensing rate >100 prescriptions per 100 residents in 2015
county.disp.map %>% 
...
# 20 counties with highest dispensing rates in 2015
county.disp.map %>% 
...
The proportion of counties where opioid dispensing rates exceeded 100 prescriptions per 100 people fell nearly 10-fold by 2020. Of the 3,049 counties reporting data, only 112 (3.7%) had at least one opioid prescription filled for every resident. The counties with the highest rates of prescribing remained in Virginia and Kentucky.

```{r eval = TRUE, message = FALSE, warning = FALSE}
# number and percent of counties with dispensing rate >100 prescriptions per 100 residents in 2020
county.disp.map %>% 
...
# display opioid prescribing rate maps (2015-2020) in a grid
grid.arrange(disp.2015, disp.2016, disp.2017, disp.2018, disp.2019, disp.2020, 
             top = textGrob("Opioid Prescriptions Dispensed per 100 Residents",
...
While the vast preponderance of counties reported a decline in prescription opioid dispensing from 2015 to 2020, the magnitude of change was variable across the country. 
...
Several counties in the Great Plains region and Inner Midwest had appreciable relative increases in opioid dispensing over the 5-year period. As shown in the table below, th large relative increases in some of the counties with the highest percent increases were attributable to the very low dispensing rates recorded in 2015 (i.e., <3 prescriptions per 100 residents per year) where the absolute increases were not meaningful. However, some counties did see appreciable relative and absolute increases in the prescription opioid dispensing rate, such as Jerauld County, SD (0.2 to 108.1 prescriptions per 100 residents), Stanton County, KS (1.5 to 105.8 prescriptions per 100 residents) and Lafayette County, AR (1.7 to 91.7 prescriptions per 100 residents).
...
[National CDC data](https://www.cdc.gov/nchs/data/databriefs/db394-tables-508.pdf) shows that the age-adjusted drug overdose death rate in the US increased 73.6% from 16.3 deaths per 100,000 people in 2015 to 28.3 per 100,000 people in 2020. In 2015, only 18 counties (0.6%) in the US had a drug overdose death rate >50 per 100,000 per year, nearly all of which were in West Virginia and Kentucky. But by 2020, 166 counties (5.3%) had a drug overdose death rate >50 per 100,000. While many counties in West Virgina and Kentucky remained the places with the highest overdose death rates in the US, the high overdose death rates in major cities like Baltimore and St. Louis were notable. There was also considerable regional variability in the overdose death rate across all years. The counties with the highest overdose death rates in 2015 and in 2020 tended to be in Appalachia. However, by 2020, drug overdose death rates had spiked in the Southwest, Gulf Coast, South Florida, and New England, as well.
...
# counties with highest drug overdose death rates in 2015
nchs.map %>% 
  filter(Year==2015) %>% 
...
# counties with highest drug overdose death rates in 2020
nchs.map %>% 
  filter(Year==2020) %>% 
  select(County, Model.based.Death.Rate, 2) %>% 
  arrange(-Model.based.Death.Rate) %>% 
...
# choropleth map of drug overdose death rates (2015)
od.2015 <- ggplot() +
  geom_sf(data = subset(nchs.map, Year==2015), 
          aes(geometry = geometry, fill = deathrate.trunc), lty=0) +
  my_theme() +                                          
  ggtitle("2015") + 
  scale_fill_viridis(name = "Drug Overdose Deaths \nper 100,000 people",      
                    labels = c("0", "20", "40", "≥60"),
                    breaks = c(0, 20, 40, 60), option = "rocket", direction = -1) + 
  theme(legend.title = element_text(size = 7), 
               legend.text = element_text(size = 7))

# choropleth map of drug overdose death rates (2016)
od.2016 <- ggplot() +
...
# choropleth map of drug overdose death rates (2017)
od.2017 <- ggplot() +
...
# choropleth map of drug overdose death rates (2018)
od.2018 <- ggplot() +
...
# choropleth map of drug overdose death rates (2019)
od.2019 <- ggplot() +
...
# choropleth map of drug overdose death rates (2020)
od.2020 <- ggplot() +
...
# display drug overdose death rate maps (2015-2020) in a grid
grid.arrange(od.2015, od.2016, od.2017, od.2018, od.2019, od.2020, 
             top = textGrob("Drug Overdose Deaths per 100,000 People",
             gp = gpar(fontsize=18, font=2)))
```

## Predictors of Drug Overdose Death Rate
### Bivariate Analyses
The distribution of the county-level drug overdose death rates was highly right-skewed, which violates a key assumption of linear models. A log transformation of the drug overdose death rate normalized the distribution to facilitate linear modeling.

```{r eval = TRUE, message = FALSE, warning = FALSE, fig.height=6, fig.width=12}
# histogram of non-transformed drug overdose death rate (2020)
hist.death.rate <- ggplot(risk.data, aes(x=Model.based.Death.Rate)) + geom_histogram() + theme_classic() + 
  scale_x_continuous(name = "Drug Overdose Death Rate (2020)")

# histogram of log-transformed drug overdose death rate (2020)
...
# display 1x2 grid of histograms of non- and log-transformed drug overdose death rates
grid.arrange(hist.death.rate, hist.log.death.rate, ncol=2, heights=5,
             top = textGrob("Rationale for Log Transform of Drug Overdose Death Rates"))
```
...
# scatterplot of log drug overdose death rate on prescription opioid dispensing rate with loess line
ggplot(risk.data, aes(disp.rate.2020, logdeathrate)) +
  ...
  geom_smooth(se=FALSE) + ylab("Log Drug Overdose Death Rate in 2020") +
  theme_classic(base_size=13)

# scatterplot of log drug overdose death rate on prescription opioid dispensing rate with regression line
ggplot(risk.data, aes(disp.rate.2020, logdeathrate)) +
  geom_point() + xlab("Prescription Opioid Dispensing Rate per 100 Residents in 2020") +
  geom_smooth(method='lm', se=FALSE) + ylab("Log Drug Overdose Death Rate in 2020") +
  theme_classic(base_size=13)
```

As previously noted, prescription opioid prescribing rates were rescaled to reflect a 10 prescription per 100 resident increase within the county. The county-level association between the drug overdose death rate and log prescription opioid dispensing rate was highly statistically significant (p<0.0001). However, dispensing rates only predicted approximately 5% of the total variance in drug overdose death rates at the county level. 
...
```{r eval = TRUE, message = FALSE, warning = FALSE}
# linear regression model of log drug overdose death rate on opioid dispensing rate in 2020
mod1 <- lm(logdeathrate ~ opioid.rx.10, data=risk.data)
summary(mod1)

# transform model coefficient for interpretation
100*(exp(mod1$coefficients[2]) - 1)
```

Since the opioid dispensing rate was also right skewed, a model was also fit where both the overdose death rate and the dispensing rate were log transformed. Here, the $\beta$ coefficients from these models can be interpreted by taking the proportional increase to the power of the coefficient estimate minus 1 times 100% (i.e., for a 20% increase in the dependent variable, $(1.2^\beta - 1) * 100\%$). For this model, a 20% increase in prescriptions dispensed per 100 residents at the county level was associated with an average $(1.2^{0.13212} - 1) * 100\% = 2.4\%$ increase in the county's drug overdose death rate.

```{r eval = TRUE, message = FALSE, warning = FALSE}
# linear regression model of log drug overdose death rate on opioid dispensing rate in 2020
mod2 <- lm(logdeathrate ~ log.opioid.rx, data=risk.data)
summary(mod2)

# transform model coefficient for interpretation
100*(1.2^mod2$coefficients[2] - 1)
```

Regardless of the distributional assumptions underlying the linear model, prescription opioid prescribing was positively associated with drug overdose death, predicting approximately 5% of the variance across counties. 

```{r eval = TRUE, message = FALSE, warning = FALSE, fig.height=6, fig.width=9}
# scatterplot of log drug overdose death rate on log prescription opioid dispensing rate with loess line
ggplot(risk.data, aes(log.opioid.rx, logdeathrate)) +
  geom_point() + xlab("Log Prescription Opioid Dispensing Rate per 100 Residents in 2020") +
  geom_smooth(se=FALSE) + ylab("Log Drug Overdose Death Rate in 2020") +
  theme_classic(base_size=13)

# scatterplot of log drug overdose death rate on log prescription opioid dispensing rate with regression line
ggplot(risk.data, aes(log.opioid.rx, logdeathrate)) +
  geom_point() + xlab("Log Prescription Opioid Dispensing Rate per 100 Residents in 2020") +
  geom_smooth(method='lm', se=FALSE) + ylab("Log Drug Overdose Death Rate in 2020") +
  theme_classic(base_size=13)
...
The CDC PLACES dataset contains age-adjusted prevalence estimates for various indicators of health within each county. The variables of interest from the dataset were:
...
```{r eval = TRUE, message = FALSE, warning = FALSE}
# summary table for prevalence of health indicator variables from PLACES dataset
...
# apply kable styles to summary table
...
# scatterplots on log drug overdose rate for each respective health indicator
...
# display grid of scatterplots of log drug overdose rate on prevalence of health indicators
...
# apply kable styles to table of most important health indicators as predictors 
kbl(custom.table, format = "html", align = 'l', 
    caption = "Summary of Health Indicators with Highest Predictive Power in Univariable Models") %>% 
  kable_styling(full_width = FALSE, position = "center", htmltable_class = "lightable-classic")
```

The correlation matrix below shows the colinearity of the opioid dispensing rate and the 6 health indicators that predicted at least 5% of the total variance in drug overdose death rates. The prescription opioid dispensing rate was not highly correlated with the prevalence of any of the important health indicators. The prevalence of depression was highly correlated with the prevalence of poor recent mental health ($\rho$ = 0.82). The prevalence of poor self-reported physical health was highly correlated with both current smoking status ($\rho$ = 0.82) and poor self-reported mental health ($\rho$ = 0.86). 

```{r eval = TRUE, message = FALSE, warning = FALSE}
# select variables for opioid dispensing rate and strongest health predictors
...
# display correlation matrix of opioid dispensing rate and strongest health predictors
...
Self-reported poor physical health and poor mental health status were removed from consideration in the final model due to high VIFs and insignificant predictors were also removed from consideration. Three independent predictors remained: prescription opioid dispensing rate, depression prevalence, and binge drinking prevalence. However, the finding that binge drinking was associated with a lower rate of drug overdose death was counterintuitive in that substance abuse of one substance would be expected to be positively assoicated with the abuse of other substances. In evaluating those counties with the highest binge drinking prevalence proportions, all top 20 counties were in Wisconsin and each county had a lower drug overdose death rate than the national average. Thus, the inverse association between binge drinking prevalence and drug overdose death rate was spurious, driven by distinct regional patterns of alcohol abuse rather than a protective effect of binge drinking.

```{r eval = TRUE, message = FALSE, warning = FALSE}
# counties with highest binge drinking prevalence proportions in 2015
risk.data %>% 
...
Therefore, the final multivariable model included two predictor variables: opioid dispensing rate and the prevalence of depression. The model results suggest that holding the other variable constant:
...
To ensure that the final model by exploratory analysis was adequately parameterized, elastic net regression was used to evaluate other potential model fits including all predictor variables of interest (with the exception of _Binge Drinking_ which was found to be confounded by state). The model was fit using the repeated cross-validation method with 5 folds repeated 5 times. The optimal value for $\alpha$ selected by the model was 0.056. (An $\alpha$ value of 0 is equivalent to Ridge regression and an $\alpha$ value of 1 is equivalent to Lasso regression, so the elastic regression model approximated a Ridge regression.) The optimal value of $\lambda$ was 0.00335. 

The cross-validated elastic net regression model did not exclude any of the predictor variables. Relative to the multivariable model generated via exploratory analysis, the parameter estimates for a 10-person increase in the prescription opioid dispensing rate more than doubled (2.9% vs 7.8% increase in drug overdose death rate) but the parameter estimate for a 5 percentage point increase in depression was attenuated (43% vs 28% increase in drug overdose death rate). There appeared to be little advantage of the model by elastic net regression from a prediction perspective since the root mean square error (RMSE) was 0.4763 and R^2^ was 24.4%, which was nearly equivalent to the more parsimonious model selected by exploratory analysis, which had an RMSE of 0.4818 and R^2^ of 22.6%. Importantly, the elastic net regression model results confirmed that the prevalence of depression was of greater predictive importance than the prescription opioid dispensing rate as shown in the importance plot below.
...
# importance plot of predictor variables in final model
plot(varImp(elastic.net.model,scale=TRUE))
```

# Conclusion
The key finding from these analyses is that regional prescription opioid prescribing is still a significant predictor of drug overdose deaths but only accounts for 5% of the total variance in overdose death rates across counties in the US. The county-level prevalence of poor mental health or depression independently predicts approximately 4 times more variance in drug overdose death rates than prescription opioid dispensing rates. This finding is congruent with the observation that the number of opioid overdose deaths attributable to prescription opioids remained unchanged between 2015 and 2020 despite a more than 40% decline in prescribing. The lack of change in overdose deaths due to prescription opioids likely reflects that prescription opioids can still be easily procured by individuals for whom prescription opioids are the opioid of choice for abuse. Notwithstanding the national and regional trends, efforts to reduce unnecessary exposure to opioids among individuals in the community remains the best solution for reducing incident cases of opioid use disorder.

While the opioid epidemic is a national problem, other findings from these analyses underscore the importance of understanding regional dynamics and substance abuse patterns. For example, while opioid abuse is typically associated with the abuse of other drugs and alcohol at the individual level, the prevalence of binge drinking at the county level was negatively associated with drug overdose death rates. This association was driven by the very high prevalence of binge drinking in Wisconsin where drug overdose death rates were below the national average. Large cohort studies or case-control studies, which can evaluate individual-level factors, would likely be more informative than these county-level summary data in identifying the health factors that predispose an individual person to opioid abuse. However, the strong positive association between poor mental health and depression with drug overdose death rates in this analysis is consistent with prior research on risk factors for opioid abuse ([Manchikanti et al, 2007](https://www.wmpllc.org/ojs/index.php/jom/article/view/1172); [Sullivan, 2018](https://www.ingentaconnect.com/content/wk/cjpn/2018/00000034/00000009/art00010)).

A recent paper by [Aubry and Carr (2022)](https://www.frontiersin.org/articles/10.3389/fpain.2022.884674/full) reported on an analysis of prescription opioid sales and prescription opioid deaths on the state level from 2010 to 2019. Their analysis regressed the annual prescription opioid sales on the opioid overdose death rate by state and found a significant negative relationship in most states (i.e., _higher_ opioid doses dispensed per capita were associated with _lower_ overdose death rates). The present analysis refutes those findings, which are attributable to the fact the authors regressed two trended time series, which is known to induce spurious correlations ([Neifeld, 1927](https://www.tandfonline.com/doi/abs/10.1080/01621459.1927.10502965?journalCode=uasa20)). In this case, opioid overdose deaths were increasing over time as a result of the proliferation of fentanyl and other synthetic opioids while prescription opioid dispensing was declining over the same time frame. While there are several statistically appropriate methods for evaluating two trended time series, the analyses presented in this report remove the influence of time entirely to demonstrate that higher opioid prescribing rates do, indeed, remain positively associated with drug overdose death.

Overall, the results of this project suggest that reducing unnecessary prescription opioid exposure should remain a public health priority. In the context of the current state of the opioid epidemic, efforts to reduce the supply of illicit fentanyl and the prevalence of mental health issues leading to substance abuse may be the most impactful interventions for reducing opioid overdose deaths.