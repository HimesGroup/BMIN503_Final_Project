...
I'll start by getting an overview of demographics within the database stratified by vaccination status (i.e., received at least one vaccine). 
```{r, eval = TRUE, message = FALSE}
...
I'll start by getting demographics, explore the data visually, and then create the table summary of the demographics stratified by vaccination status.
...
dems %>% 
  ...
  bold_labels()
```
I'll do the same approach for the insurance status.
...
```
Next, I'll get and explore the comorbidity data.
...
```
Lastly, I'll get the outcomes, explore the data visually, and then create the table summary of the outcomes stratified by vaccination status. Note that all outcomes include corresponding dates except death which is only recorded by month. 
