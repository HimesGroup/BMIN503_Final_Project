...
cl <- cont.chart.data %>% 
  filter(arr_date <= ymd('2019-10-15',tz='UTC')) %>% # for the center line, we only want data through the data of the intervention
  mutate(time = arr_yr_wk) %>%
  group_by(time) %>%
  summarise(
    # n_ss = sum(ss_used_med == 'No Order Set'),
...
if (cohort <= 5000) {
  top.meds <- c('Amoxicillin','Amoxicillin Clavulanate','Cefdinir','Azithromycin') # patch so multicloss roc curves work for sample data
}
