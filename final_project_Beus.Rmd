...
Adherence to the guidelines was much lower in patients >= 2 years of age.
...
# logistic regression for association between adherence and antibiotic
f <- glm(dur_compl ~ abx_name, family = binomial, data = coh.sub)
broom::tidy(f, exponentiate = T, conf.int = T) # the meaning of the estimate for year-mo would be difficult to interpret
...
# logistic regression for association between adherence and provider type
f <- glm(dur_compl ~ prov_rx, family = binomial, data = coh.sub)
broom::tidy(f, exponentiate = T, conf.int = T)
...
# patch so multiclass roc curves work for sample data
if (nrow(cohort) <= 5000) {
...
Briefly summarise relationship between antibiotic choice and covariates. Not many of the covariates appear to be correlated with antibiotic choice. As might be expected, the presence of allergies seems to be related to antibiotic choice.
...
var_names <- abx.coh %>%
  ...
  pivot_longer(cols = race_f:cef_all,
               values_to = 'vals') %>%
  pull(name) %>%
  unique()

# used to make nicer labels for variables in facetes
var_labels <- c('Race',
                'Age',
                'Sex',
                'Medical Complexity',
...
abx.coh %>%
  ...
  group_by(name, vals) %>%
  mutate(n_val = n()) %>%
  group_by(name, vals, abx_name) %>%
  mutate(n_abx = n()) %>%
  mutate(
    prop = n_abx/n_val
  ) %>%
  distinct() %>%
  arrange(name, vals, abx_name) %>%
  ggplot(aes(x=vals, y=prop, fill=abx_name)) +
  geom_col() + 
  scale_x_discrete(label = function(x) stringr::str_trunc(x, 12)) + #shorten x-axis labels
  scale_fill_discrete('Antibiotic Name') +
...
Kappa is highest for the model using extreme gradient boosting. While accuracy is highest for the Naive Bayes model, we see that this model just predicts the majority class (amoxicillin) for every observation. 
...
#### Multiclass ROC and PR Curves for ML Models (Test Data)
...
# extract probablities for each class for each method using testing data
...
# get ROC and Precision-recall data for testing data
