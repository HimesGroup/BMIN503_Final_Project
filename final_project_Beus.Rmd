...
1. comment on LR and ML models
1. do we need to create dummy volumes
1. create fake data to allow report to run
1. confirm works with test data - comment out PHI data pulls
1. show run chart after intervention ?
  1. balancing measures
    1. change in antibiotic use (non-amox)
    1. change in smart-set use (? shift-away)
...
* Patient Age
* Provider Type
* Whether the order set was used

Smaller effects were observed for:

* Time of day (particularly late morning)
* Day of the week
...
# Discussion of Logistic Model Results

**further discuss results above**
...
set.seed(54321)
idxtrain <- caret::createDataPartition(y = abx.coh$abx_name, p = 0.7, list =F)
train <- abx.coh[idxtrain,]
testing <- abx.coh[-idxtrain,]

pp <- preProcess(train %>% select(age_yr, arr_yr), c('scale','center','nzv')) # scale and center continuous variables

transformed <- predict(pp, newdata = train)

pp.test <- preProcess(testing %>% select(age_yr, arr_yr), c('scale','center','nzv')) # scale and center continuous variables

trans.test <- predict(pp.test, newdata = testing)
```

Create Balanced Sample with respect to antibiotic name
```{r}

# abx.bal <- caret::upSample(train, train$abx_name, yname='abx_name') # create balanced classes with respect to antibiotics

```

```{r}

# mlr <- nnet::multinom(abx_name ~ ., data = train)
# summary(mlr)
# 
# mlr.pred <- predict(mlr, newdata = testing)
# 
# confusionMatrix(mlr.pred, testing$abx_name)
...
Preprocess numeric data and establish training method.
```{r cache=F}
...
tr_control <- trainControl(## 10-fold CV
                           ...
                           repeats = 3,
                           verboseIter = T,
                           sampling = 'down') # upsampling would be preferable so as not to drop data. it did not result in significantly better results, so for performance (time spent fitting models) reasons, will use down sampling
...
# confusionMatrix(predict(knn.abx), train$abx_name)
confusionMatrix(predict(knn.abx, newdata = trans.test), testing$abx_name)
...
# confusionMatrix(predict(pmr.abx), train$abx_name)
confusionMatrix(predict(pmr.abx, trans.test), testing$abx_name)
...
confusionMatrix(predict(nb.abx, trans.test), testing$abx_name)
...
confusionMatrix(predict(rf.abx, trans.test), testing$abx_name)
...
Accuracy higher for naive bayes, although this results in significantly lower detection rates for non-amoxicillin medications.
