...
#Specific Exposure Interest is in Xylenes - 3 major metabolites
#Pull variables of interest - age, gender, race, education, income, occupation
nhanes.practice <- nhanes.data |>
  select(id = SEQN, gender = RIAGENDR, age = RIDAGEYR, 
           race = RIDRETH1, education = DMDEDUC2, income = INDHHIN2,
           DPMA = URXDPM.x, MHA2 = URX2MH.x, MHA34 = URX34M.x, 
           current_industry = OCD231.y) |>
...
#descriptive stats 
#table 
#distribution of VOC (general idea of all of them) 
```
...
```{r}
d1 <- ggplot(data = nhanes.practice, aes(x = race)) +
        geom_bar()
d2 <- ggplot(data = nhanes.practice, aes(x = education)) +
        geom_bar()
d3 <- ggplot(data = nhanes.practice, aes(x = income.short)) +
        geom_bar()
d4 <- ggplot(data = nhanes.practice, aes(x = current_industry)) +
        geom_bar()
