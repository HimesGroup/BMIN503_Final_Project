...
I read in the Sparta scan data file (containing all sports) and loaded the appropriate tidyverse packages, above:
library(dplyr)
library(ggplot2)
...
I wanted to focus on just one sport for this project, and so I looked at the total number of scans we had for each sport in the entire dataset and confirmed that it's Men's lacrosse (we knew that).
```{r}
...
We're working with 35 Men's lacrosse players with 852 jump scans from 2016-2020 (athletes with complete data).
...
Below, we see that athletes a lot of variability in each athlete's LOAD, EXPLODE, DRIVE - across the board, athletes had scans where they were Low, Medium, High on each variable.
...
See Finalproject_R script for draft
