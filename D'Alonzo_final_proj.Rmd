...
To begin, I loaded the appropriate tidyverse packages and read in the Sparta Science scan data file (containing all sports). I also used the package gtsummary to create summary tables.
...
I aimed to focus on a single sport for this project, and so I first examined the total number of scans that we had for each sport in the Sparta dataset. I confirmed that Men's Varsity Lacrosse has the most (Note, we suspected this following conversations with the Strength & Conditioning group regarding this team's frequent utilization of Sparta and buy-in from coaches, etc.).
```{r, message=FALSE}
...
Next, I cleaned the dataset, removing identifiers. I restricted the sample to Men's Lacrosse and Counter Movement Jump (CMJ) scan only. I assigned a new unique ID to each athlete (id.new):
```{r}
...
### Results
We are now looking at 35 Men's lacrosse players with 852 jump scans from 2016-2020 (athletes with complete data).
...
We explore "Injury Risk Score" first, which ranges from 0-low to 5-high risk of injury. We see that the majority of scans (47%; almost half) were  reported with an Injury Risk Score of 1, followed by 3, then 0. We can further see that Injury Risk Score varies greatly both across the 35 lacrosse athletes and within a given athlete here. Almost all athletes experienced each Injury Risk Score at one point in time. A next step in the larger study will be to see whether an increase (or change) in Injury Risk Score for a given athlete coincides with an injury occurring by merging this dataset with medical/athletic training records containing injuries.
```{r}
...
We look at Injury Risk Score and Jump Height below, and see that in general, higher jumps seem to coincide with lower Injury Risk Scores; 0 and 2, however, Jump Height is fairly similar across all Scores.
...
We explore LOAD, EXPLODE, and DRIVE tscores; the 3 variables that make up the Sparta Movement Signature. Again, we see variability in the scans across the 35 athletes and over time, within the same athlete.
```{r, message=FALSE}
library(gridExtra)
```
...
#ggplot(data = sparta.scans.clean, aes(x = load.tscore, y = id.new)) +
  #geom_point()
box.loadt <- ggplot(data = sparta.scans.clean, aes(x = load.tscore, y = id.new)) +
  geom_boxplot() +
  labs(y = "Athlete ID",
         x = "Load t-score") +
  ggtitle("Load")
...
#ggplot(data = sparta.scans.clean, aes(x = explode.tscore, y = id.new)) +
  #geom_point()
box.explodet <- ggplot(data = sparta.scans.clean, aes(x = explode.tscore, y = id.new)) +
  geom_boxplot() +
  labs(y = ".",
       x = "Explode t-score") +
  ggtitle("Explode")
...
#ggplot(data = sparta.scans.clean, aes(x = drive.tscore, y = id.new)) +
  #geom_point()
box.drivet <- ggplot(data = sparta.scans.clean, aes(x = drive.tscore, y = id.new)) +
  geom_boxplot() +
  labs(y = ".",
       x = "Drive t-score") +
  ggtitle("Drive")
...
We then examine the relationship between LOAD, EXPLODE, DRIVE and injury risk score. We see that low Injury Risk Score (0,1) seems to coincide with tscores primarily between 45-60, while higher Injury Risk Scores are clustered at tscores below 45 and above 60.
```{r}
...
In order to better conceptualize LOAD, EXPLODE, and DRIVE scores as one measure, I generated a variable similar to Sparta Science's Movement Signature. I split each variable LOAD, EXPLODE, and DRIVE into tertiles and classified each scan according to "High," "Medium," "Low" on each measure. I then concatenated them, creating a "Jump Profile" for each jump scan.
...
Below, we see a lot of variability within each athlete's LOAD, EXPLODE, DRIVE. Across the board, athletes had scans that could be classified as Low, Medium, and High, consistently across all 3 measures. We expected there to be some variation in scores across athletes, but we were surprised to see such variation within an athlete. Again, like with the original tscores, a next step in this project will be to see if change in Jump Profile coincides with actual injury based on injury records.

```{r}
#This shows variability in LOAD, EXPLODE, DRIVE within and across athletes
#Load
bar.loadprofile <- ggplot(data = sparta.scans.clean, aes(y = id.new, fill = load.tert)) +
...
#Explode
bar.explodeprofile <- ggplot(data = sparta.scans.clean, aes(y = id.new, fill = explode.tert)) +
  geom_bar() +
  labs(y = ".",
       x = "Scans (count)",
       fill = "Tertiles") +
...
#Drive
bar.driveprofile <- ggplot(data = sparta.scans.clean, aes(y = id.new, fill = drive.tert)) +
  geom_bar(show.legend = FALSE) +
  labs(y = ".",
       x = "Scans (count)",
       fill = "Drive tertiles") +
...
I then fit a multivariable linear regression model:
