...
geo_format <- function(df){
  fmr_geo <- counties %>%
    dplyr::select(c("NAME", "ID", "geometry")) %>%
    left_join(df[,c("ID","fmr0", "fmr1", "fmr2", "fmr3", "fmr4",
...
# add geographical data to all FMRs 
...
fmr1_2017_map <- map_rent(fmr_2017_geo, 2018) # generate static rent map
...
# this funciton allows you to plot interactive leaflet maps of median US rents by year
...
title <- tags$div(
  ...
  interactive_map <- leaflet(df) %>%
    ...
    addLegend("bottomright",                           # location of legend
            ...
            title = paste("Rent",1,"bed (2010 $)"),  # legend title
...
fmr_2017_mapi <- map_rent_i(fmr_2017_geo, 2017) # display leaflet map
fmr_2017_mapi
