...
weight <- function(year, variable, title){
  ...
  ggplot() +
    ...
    ggsave(paste("./img/", variable, "_", year, ".png", sep=''))                    #saves as a png file so they can be combined and animated later
}
...
#Not the best way to display this data, but wanted to play with R's ability to animate.
system("convert -delay 150 ./img/*.png ./img/weight.gif")
```
<img src="./img/weight.gif" />

#### States colored in black are those that did not report data for the survey
...
Our overall goal was to see what impact our intervention would have on the patients overall health. We measured these through validated, standardized surveys, including the Pediatric Quality of Life Inventory ([https://www.pedsql.org/ "PedsQL"]) and the National Youth Physical and Activity Nutrition Study Survey ([https://www.cdc.gov/healthyyouth/data/yrbs/nypans.htm "NYPANS"]). With these measures we assess our impact on changing the patients behaviors and feelings in regard to diet, exercise and their weight (although as discussed later, in future studies it would be better to focus less on weight). The PedsQL also gives us an assessment of how the child assess their overall well-being.
...
```{r}
#combined language collumns

#first two columns are for the patient's preferred language
languages <-function(info){
  ...
  #second columns are for the parent's preferred language
...
plotting <- function(ins, pre, post) {
  ...
  border <- if_else(is.na(str_extract(ins, "Control")), "darkorchid4", "deeppink")
  
  ggplot() +                    #dates are converted back to actual dates below, I had been working with them as pure numbers
    geom_point(data = subset(pre, redcap_id==ins), aes(x=date, y=weight), color="firebrick1") +
    geom_point(data = subset(post, redcap_id==ins), aes(x=date, y=weight), color="blue4") +
    geom_segment(aes(x = as.Date(precoeffs[1], "2014-01-01"), y =precoeffs[2], xend=as.Date(precoeffs[3], "2014-01-01"), yend=precoeffs[4], color = "blue4"), data=pre) +
    geom_segment(aes(x = as.Date(postcoeffs[1], "2014-01-01"), y =postcoeffs[2], xend=as.Date(postcoeffs[3], "2014-01-01"), yend=postcoeffs[4], color = "firebrick1"), data=post) +
    theme(legend.position="none",
          ...
          panel.border = element_rect(colour = border, fill = NA, size = 2))
}
...
#### Linear regression was run on the the weight of the participants and on controls before and after the intervention started. Below is displayed the recorded weights for each study participant as well as the controls over time. Their regression line is also displayed with them, red for pre-study, blue for post-study, to allow a visual representation of weight change surrounding the intervention. Purple border highlights the study participants, pink border highlights controls.
```{r warning=FALSE, fig.height=20, fig.width=30}
