...
# ggplot(data = AQI.ACS.geom, aes(x = longitude, y = latitude, color = pm2.5_p50)) + 
#     geom_point() + 
#     borders("state") + 
...
library(leaflet)
AQI.ACS.geom.WGS84 <- st_transform(AQI.ACS.geom, 4326)
...
pal_fun3 <- colorNumeric("viridis", NULL, reverse=TRUE)   # viridis from viridis
...
# Median PM2.5
popup_msg <- paste0(AQI.ACS.geom.WGS84$county
                    ," County, "
                   ,AQI.ACS.geom.WGS84$state
                   ,", "
                   ,AQI.ACS.geom.WGS84$NAMELSAD
                   ,"<br>Metro Area: "
                   ,AQI.ACS.geom.WGS84$cbsa
                   ,"<br>Median PM2.5: "
                   ,round(AQI.ACS.geom.WGS84$pm2.5_p50,digits=2))

leaflet(AQI.ACS.geom.WGS84) %>%
  addPolygons(fillColor = ~pal_fun3(pm2.5_p50)
              ,fillOpacity = 0.9
              ,weight = 1.5
              ,color = "black"
              ,popup = popup_msg) %>%      
...
# % of days above 12 mcg/m3
popup_msg2 <- paste0(AQI.ACS.geom.WGS84$county
                    ," County, "
                   ,AQI.ACS.geom.WGS84$state
                   ,", "
                   ,AQI.ACS.geom.WGS84$NAMELSAD
                   ,"<br>Metro Area: "
                   ,AQI.ACS.geom.WGS84$cbsa
                   ,"<br>Reached 12mcg/m3 on "
                   ,round(AQI.ACS.geom.WGS84$pm2.5_pct_ungreen,digits=1)
                   ,"% of days")

leaflet(AQI.ACS.geom.WGS84) %>%
  addPolygons(fillColor = ~pal_fun3(pm2.5_pct_ungreen)
              ,fillOpacity = 0.9
              ,weight = 1.5
              ,color = "black"
              ,popup = popup_msg2) %>%      
  ...
  addLegend("bottomright",         
            ...
            title = '% days reaching 12mcg/m3',  
            opacity = 1) %>%    
  addScaleBar()
```


Multivariate linear models and logistic regressions for binary outcomes

```{r modeling-1, eval=TRUE}
lm.p50 <- lm(pm2.5_p50 ~ 
...
summary(lm.p50)$coefficients
summary(lm.p50)$coefficients[-1,4]

lm.p50.pval.df <- data.frame(acs.var = names(summary(lm.p50)$coefficients[-1,4])
                             ,lm.p50.pval  = summary(lm.p50)$coefficients[-1,4])
lm.p50.pval.df

summary(lm.pct.ungreen)$coefficients
summary(lm.pct.ungreen)$coefficients[-1,4]
...
lm.pct.ungreen.pval.df


summary(logit.flag10.ungreen)
summary(logit.flag10.ungreen)$coefficients
...
logit.flag10.ungreen.pval.df

summary(logit.flag20.ungreen)
summary(logit.flag20.ungreen)$coefficients
...
ggplot(data = AQI.ACS.geom, aes(x = worked.12mo       , y = pm2.5_p50)) + 
  geom_point() + geom_smooth(color = "red", method = "lm") + labs(y="Median daily PM 2.5 concentration (mcg/cubic meter)")
...
ggplot(data = AQI.ACS.geom, aes(x = worked.12mo           , y = pm2.5_pct_ungreen)) + 
  geom_point() + geom_smooth(color = "blue", method = "lm") + labs(y="Percent of days above 12mcg/cubic meter of PM 2.5")
ggplot(data = AQI.ACS.geom, aes(x = manufacturing         , y = pm2.5_pct_ungreen)) + 
  geom_point() + geom_smooth(color = "blue", method = "lm") + labs(y="Percent of days above 12mcg/cubic meter of PM 2.5")
ggplot(data = AQI.ACS.geom, aes(x = ethn.nhisp.white.alone, y = pm2.5_pct_ungreen)) + 
  geom_point() + geom_smooth(color = "blue", method = "lm") + labs(y="Percent of days above 12mcg/cubic meter of PM 2.5")
ggplot(data = AQI.ACS.geom, aes(x = farm.fish.mining,       y = pm2.5_pct_ungreen)) + 
  geom_point() + geom_smooth(color = "blue", method = "lm") + labs(y="Percent of days above 12mcg/cubic meter of PM 2.5")

ggplot(data = AQI.ACS.geom, aes(x = pm2.5_flag10_ungreen, y = manufacturing)) +
    geom_violin(fill = "springgreen1", draw_quantiles = c(0.25, 0.5, 0.75)) + labs(x="Percent of days above 12mcg/cubic meter of PM 2.5")
...
ggplot(data = AQI.ACS.geom, aes(x = pm2.5_flag10_ungreen, y = farm.fish.mining)) +
    geom_violin(fill = "springgreen1", draw_quantiles = c(0.25, 0.5, 0.75)) + labs(x="Percent of days above 12mcg/cubic meter of PM 2.5")
ggplot(data = AQI.ACS.geom, aes(x = pm2.5_flag10_ungreen, y = snap.12mo)) +
    geom_violin(fill = "springgreen1", draw_quantiles = c(0.25, 0.5, 0.75)) + labs(x="Percent of days above 12mcg/cubic meter of PM 2.5")

ggplot(data = AQI.ACS.geom, aes(x = pm2.5_flag20_ungreen, y = worked.12mo)) +
    geom_violin(fill = "steelblue1", draw_quantiles = c(0.25, 0.5, 0.75)) + labs(x="Percent of days above 12mcg/cubic meter of PM 2.5")
ggplot(data = AQI.ACS.geom, aes(x = pm2.5_flag20_ungreen, y = ethn.nhisp.white.alone)) +
    geom_violin(fill = "steelblue1", draw_quantiles = c(0.25, 0.5, 0.75)) + labs(x="Percent of days above 12mcg/cubic meter of PM 2.5")
ggplot(data = AQI.ACS.geom, aes(x = pm2.5_flag20_ungreen, y = income.hh.median.10k)) +
    geom_violin(fill = "steelblue1", draw_quantiles = c(0.25, 0.5, 0.75)) + labs(x="Percent of days above 12mcg/cubic meter of PM 2.5")
ggplot(data = AQI.ACS.geom, aes(x = pm2.5_flag20_ungreen, y = bachelor.plus)) +
    geom_violin(fill = "steelblue1", draw_quantiles = c(0.25, 0.5, 0.75)) + labs(x="Percent of days above 12mcg/cubic meter of PM 2.5")
...
ggplot() +
  geom_sf(data = AQI.ACS.geom, aes(fill=pm2.5_p50)) +
  borders("state")
  
ggplot() +
  geom_sf(data = AQI.ACS.geom, aes(fill=pm2.5_p50)) +
  ggtitle("Median daily PM2.5 reading for 2019 by census tract") + 
  scale_color_gradient(low = "#ffff00", high = "#0000ff") +
  borders("state")
