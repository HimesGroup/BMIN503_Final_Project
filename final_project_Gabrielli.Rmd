...
head(NH_alive6m)
sum(is.na(NH_alive6m)) #535 NA values in deficits
...
table.imp.alive6m <- data.frame(imp.alive6m[,4]) %>%
  mutate(table.imp.alive6m[[,1]]) = 
colnames(table.imp.alive6m)[c(1)] <- c("Alive6m.MeanDecreaseGini")
...
table.imp.FLP <- data.frame(imp.FLP[,4])
colnames(table.imp.FLP)[c(1)] <- c("FLP.MeanDecreaseGini")
table.imp.FLP
...
table.imp.FFO <- data.frame(imp.FFO[,4])
colnames(table.imp.FFO)[c(1)] <- c("FFO.MeanDecreaseGini")
table.imp.FLP
...
6 month Survival: All models predicting 6 month survival had a moderate-fair performance (Figure 4a,4b)(Table 2). The Logistic Regression model using top variables performed slightly better than other models with an AUC of 0.67. There is evidence for overfitting in both models of random forests as shown by the large difference between the AUC of the training versus testing data. 

Final Living Place: All models predicting 6 month survival had a moderate performance (Figure 5a,5b)(Table 3). The Logistic Regression model using top variables again performed slightly better than other models with an AUC of 0.7135. There is evidence for overfitting in both models of random forests as shown by the large difference between the AUC of the training versus testing data.

Final Functional OUtcome: All models predicting 6 month survival had a moderate-fair performance (Figure 6a,6b)(Table 4). The Logistic Regression model using top variables again performed slightly better than other models with an AUC of 0.71. There is evidence for overfitting in both models of random forests as shown by the large difference between the AUC of the training versus testing data.
...
#Figure 4a. ROC curves of RF with all variables in alive at 6 m outcome
pROC::plot.roc(NH_alive6m$alive6m, rf.pred.alive6m, ci=TRUE, col="darkgreen", add=FALSE)
title("Figure 4a. ROC curves of RF with all variables in alive at 6 m outcome")
...
plot.roc(NH_alive6m$alive6m, alive6m.top.glm.pred, ci+TRUE, main="Figure 4b. ROC curves of Top Variable Prediction Models for 6 Month Survival")
...
title("Figure 5a. ROC curves of RF using all variables for Final Living Place outcome")
legend("bottomright", legend=c("RF Training", "RF Cross-Validation"), col=c("darkgreen", "red"), lwd=2)

#Figure 5b. ROC curves of RF and GLM predictors of Final Living Place (FLP) with top variables
plot.roc(NH_FLP$FLP, FLP.top.glm.pred, ci=TRUE, main="Figure 5b. ROC curves of Top Variable Prediction Models for Final Living Place")
...
#Table 3: AUC for Final Living Place Models
...
Table3 <- cbind(Model_FLP, AUC)
Table3 #Table3: Comparison of AUC for each Prediction Model for Final Living Place
...
title("Figure 6a. ROC curves of RF using all variables for Final Functional Outcome")
legend("bottomright", legend=c("RF Training", "RF Cross-Validation"), col=c("darkgreen", "red"), lwd=2)

#Figure 6b. ROC curves of RF and GLM predictors of Final Functional Outcome (FFO) with top variables
plot.roc(NH_FFO$FFO, FFO.top.glm.pred, ci=TRUE, main="Figure 5b. ROC curves of Top Variable Prediction Models for Final Functional Outcome")
...
#Table 4: AUC for Final Functional Outcome (FFO) Models
...
Table4 <- cbind(Model_FFO, AUC)
Table4 #Table4: Comparison of AUC for each Prediction Model for Final Functional Outcome
```
