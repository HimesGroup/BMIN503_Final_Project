...
#### Aggregating smoking and lung cancer variables at CBSA level
...
```{r wtkg3}
smart_2021_d_smoker3_cbsa <- smart_2021 |>
  select(d_mmsa, d_mmsawt, d_smoker3, d_resp_id) |>
  mutate(ever_smoker = case_when(d_smoker3 %in% c(1, 2, 3) ~ TRUE,
                                 TRUE ~ FALSE)) |>
  group_by(d_mmsa, ever_smoker) |>
  summarize(n_respondents = n_distinct(d_resp_id)) |>
  ungroup() |>
  pivot_wider(id_cols = d_mmsa,
              values_from = n_respondents,
              names_from = ever_smoker,
              names_glue =  'ever_smoker_{ever_smoker}') |>
  mutate(total_respondents = across(starts_with("ever_smoker_")) |> rowSums())
```
...
adi2021_sf_4326 |> saveRDS("data/intermediate_data/adi2021_sf_4326.rds")
```
...
```{r mapadi2}
adi2021_sf_4326 <- readRDS("data/intermediate_data/adi2021_sf_4326.rds")
```
