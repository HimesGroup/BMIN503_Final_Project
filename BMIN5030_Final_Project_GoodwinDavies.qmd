...
Focussing only on the SVI and ADI measures, SVI theme 1, SVI theme 4, and ADI are associated with decreased likelihood of smoking. SVI theme 2 and SVI theme 3 are associated with decreased likelihood of smoking.
...
The figures below explore distributions of the d_ever_smoker variable. From visual inspection of the boxplots, no obvious pattern is discernable.
...
Binomial logistic regression analyses are applied to explore SVI and ADI measures as predictors of likelihood of asthma at the individual level.
...
Turning to the analyses, and focusing on SVI and ADI, higher SVI theme 1, and SVI theme 4 are associated with increased likelihood of asthma. SVI theme 3 and ADI are associated with decreased likelihood of asthma.
...
This project has many limitations. The overarching limitation is that this project presents preliminary analyses and is limited in scope given time constraints. Specific limitations are discussed below:

-   The analyses where restricted to micropolitan and metropolitan CBSAs. As such, analyses are not representative of the US and furthermore over represent urban communities. Prior work suggests that relationships between SVI and ADI (Rollings et al 2023), as well as relationships between area-level and individual-level (Xie et al 2021) differ between urban and rural communities.

-   Survey weighting was not incorporated for the individual-level analyses and as such results are not representative of the US population sampled.

-   The analyses presented are cross-sectional and observational; inferences about cause and effect are very limited. Furthermore, as with most observational analyses, there are very likely confounders for which data was not available for analyses. An example for the asthma analyses might be air quality which likely varies across geographic areas with differing socioeconomic deprivation indices.

-   For the regression analyses, future work would perform a detailed exploration of whether assumptions of the models were met and goodness of fit. For example, Lotfata et al 2023 discuss non-linear relationships in a study which examines socioeconomic and environmental determinants of asthma prevalence. For the individual-level analyses, additional individual-level predictors from the SMART BRFSS dataset which are known to have associations with asthma could be incorporated. Mixed models which allow the contributions of indices to vary by region or urbanicity may also be useful (cf. Rollings et al 2023, Lotfata et al 2023).

-   Most recent datasets were used for analyses. This resulted in some some discrepancies in data source years (2020 versus 2021).

-   Regarding outcomes, these may not have been optimally selected. For smoking, it may have been better to analyse current smoker status rather than ever smoker status for ease of comparison with CDC-provided statistics. For asthma, the more relevant outcome may have been asthma exacerbations rather than prevalence, as discussed in the previous section. The self-reported asthma variable is based on responses to a question about whether respondents had been told by a doctor, nurse, or health professional that they had asthma, which will have a relationship with access to medical care that may result in a bias whereby respondents with poorer access to medical care are less likely to report asthma. Unfortunately this data was not available in the SMART BRFSS dataset. For both smoking and asthma, the decision to group unknown and missing responses were grouped together with no responses for the area level analyses may have caused issues.
...
<!-- -   Restricted to CDC micropolitan and metropolitan CSBAs. -->
