...
```{r eval=TRUE}
dups.deleted <- bmdcs
dups.deleted <-dups.deleted[order(dups.deleted[,'visit'],-dups.deleted[,'haz']),]
dups.deleted <- dups.deleted[!duplicated(dups.deleted$visit),]
...
```{r eval=TRUE}
#All
full.ctvbmd.udBMD.all = lmer(scale(fore_r_ud_bmd_pc) ~ (1 | randid) + black + scale(age) + scale(age_squared) + scale(crt_den_r30), data = bmdcs)
...
extractAIC(full.ctvbmd.udBMD.interaction)
```
Radius Ct.vBMD is positively associated with UD radius aBMD. The interaction between Ct.vBMD and sex was investigated, but was not statistically significant. 
#####SINCE THERE'S NO SIG INTERACTION, JUST PRESENT THE TOTAL COHORT RESULTS
...
```{r eval=TRUE}
#All
full.ttar.udBMD.all = lmer(scale(fore_r_ud_bmd_pc) ~ (1 | randid) + black + scale(age) + scale(age_squared) + scale(tot_a_r30), data = new.ttar.df)
...
extractAIC(full.ttar.udBMD.interaction)
```
Tt.Ar is positively associated with UD radius aBMD in both males and females. The interaction between Tt.Ar and sex was investigated, but was not statistically significant. 
#####SINCE THERE'S NO SIG INTERACTION, JUST PRESENT THE TOTAL COHORT RESULTS
...
younger.tracking.model.df <- tidy(younger.tracking.model) # create data.frame of regression results for younger.tracking.model
medium.tracking.model.df <- tidy(medium.tracking.model) # create data.frame of regression results for medium.tracking.model
older.tracking.model.df <- tidy(older.tracking.model) # create data.frame of regression results for older.tracking.model

m1_df <- tidy(younger.tracking.model) %>% filter(term != "(Intercept)") %>% mutate(model = "Model 1")
m2_df <- tidy(medium.tracking.model) %>% filter(term != "(Intercept)") %>% mutate(model = "Model 2")
m3_df <- tidy(older.tracking.model) %>% filter(term != "(Intercept)") %>% mutate(model = "Model 3")

three_model_whisker <- rbind(m1_df, m2_df, m3_df)
...
#dwplot(list(younger.tracking.model, medium.tracking.model, older.tracking.model)) ##not sure what this one is plotting.... probably the estimate for the total model, not each individual predictor?
