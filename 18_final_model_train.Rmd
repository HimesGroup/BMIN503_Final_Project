...
for (res in results){
  ...
  densplot <- densityplot(resample_results, metric="Kappa", auto.key = list(columns=3))
  file_name = paste("kappa_", count, ".tiff", sep="")
    tiff(file_name)
    print(densplot)
    dev.off()
  # Plot kappa and acc, higher is better
  bplot_acc_kapp <- bwplot(resample_results, metric = c("Kappa","Accuracy"))
  file_name = paste("kappaacc_", count, ".tiff", sep="")
    tiff(file_name)
    print(bplot_acc_kapp)
    dev.off()
  # Plot logloss, lower is better
  bplot_logloss <- bwplot(resample_results, metric = c("logLoss"))
