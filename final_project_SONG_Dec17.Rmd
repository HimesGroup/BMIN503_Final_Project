...
knitr::opts_chunk$set(warning=FALSE, message=FALSE, error=FALSE, width = 400)
options(width = 400)
```  
...
## 1.1. Overview
The goal of my final project is to use survey data to analyze health related factors and predict potential risk of diseases. The National Health Interview Data are analyzed with logistic, rare event logistic and machine learning methods to predict risks of 16 types of comman diseases. Cross validation is applied to evaluate the power of prediction of estimated models.

## 1.2. Aims of the project
Chronic diseases such as cancer, lung disease, and stroke have been major health concerns and reasons of death in the U.S. Risk evaluation of these diseases can lead to preventional treatments or lifestyle changes that help patients survive and improve quality of life. The National Health Interview database enables us to use analytical tools to estimate the associations between lifestyles and health and predict the risks of many types of diseases with only simple interview answers. The risks of diseases could be estimated before any labs and clinical visits. When integrated into web tools, anybody can do the predictions by answering simple questions about themselves. 
...
```{r, ch2.1 }
temp <- tempfile()
download.file("ftp://ftp.cdc.gov/pub/Health_Statistics/NCHS/Datasets/NHIS/2018/househldcsv.zip", temp)
hshld <- read.csv(unz(temp, "househld.csv"))
download.file("ftp://ftp.cdc.gov/pub/Health_Statistics/NCHS/Datasets/NHIS/2018/familyxxcsv.zip", temp)
fmly <- read.csv(unz(temp, "familyxx.csv"))
download.file("ftp://ftp.cdc.gov/pub/Health_Statistics/NCHS/Datasets/NHIS/2018/personsxcsv.zip", temp)
prsn <- read.csv(unz(temp, "personsx.csv"))
download.file("ftp://ftp.cdc.gov/pub/Health_Statistics/NCHS/Datasets/NHIS/2018/samadultcsv.zip", temp)
adlt <- read.csv(unz(temp, "samadult.csv"))
download.file("ftp://ftp.cdc.gov/pub/Health_Statistics/NCHS/Datasets/NHIS/2018/samchildcsv.zip", temp)
chldrn <- read.csv(unz(temp, "samchild.csv"))
unlink(temp)
...
* Four of the 5 data tables are potentially the data sources for the study project. The child table for children information is excluded from this project because their information is unlikely to be predictors for adults' health. The remaining 4 tables have a total 72,831 rows and 1,466 columns
* Only 1 column of data for family size in the family data table will be use for this study. After rounds of data exploration, none from the household table are included.
* The personsx table includes data of all interviewed members of families. Diseases information of adults are in the adults table, and most of these diseases are also included in the personsx table but are systematically named differently from those in the adults table.
...
Potential response variables are selected diseases such as cancers and 15 other types of chronic diseases. All demographic, activities, smoking, drinking, and other lifestyle related factors are included as potential predictors. The unknown, not answered, and not applicable values are change into NA values. All categorical and some continuous variables such as first smoking age and hours of sleep are transformed into categorical variables to simplify estimation for logistic models by constructing only linear link functions in the logistic model. Such a categoricalization of continuous variables is an approximation to a nonlinear link function. All data categories are labeled to reduce errors in coding. Consistency of transformation is checked against their corresponding original data. Thus, you may expect to see some examination code lines for consistency. The three columns of data for vigorous, strength, and moderate physical activities are combined into a categorical variable to measure different types of activity. Age and BMI are the only continuous variables included in the estimation. They are kept as continuous variables just to keep the diversity of the model and see if the code may work better or worse with them. For details of the data transformations, please read the comments in the code chunks. 
...
This section uses mainly plots to represent distributions of variables.
...
p8 <- ggplot(data = XX0, aes(x = BMI)) + 
  ...
  labs(y = "People (n)", x = "BMI")  +
  theme_bw()
p9 <- ggplot(data = XX0, aes(x = fm_size)) + 
  geom_bar(color="black", fill = "Brown", width=0.7) +
  labs(y="People (n)", x="Family size")  +
  theme_bw()
...
Ratios of cases of selected disease are ploted against drinking and smoking, which are the 2 most interesting factors I would like to analyze.
...
>The above plot demonstrates that bar plots of absolute numbers of each disease categories are not suitable for comparison against the categories of other variables, Thus, bar plots of ratios will be applied. 
...
This section checked correlations between parent diseases and adult diseases.  The diseases are linked using a parents flag in the adult data.
...
## check for relationship between adult and mom
ggplot(data = hlth_par[!is.na(hlth_par$mdis11), ], aes(x = dis11, fill = mdis11)) + 
  geom_bar(position = "dodge", color="blue", na.rm = TRUE) +
  theme_bw()+
  labs(x="Mom lung problem", y="People (n)")+
  theme(legend.title = element_blank())
```

>The above plot suggests again the difficulty in displaying the diseases with absolute number count. So in this project most of the results will be shown in relative values or percents.

```{r, ch3.3_second, warning=FALSE, message=FALSE, error=FALSE}
...
r=tmt[,2]/(tmt[,1]+tmt[,2])*100
barplot(r, space =0.3,xlab = "Mom heart disease", ylab="Heart disease %", col=c("green","red"))
# in this case the sample size is too small, 1 case would be too big
...
>Parents' health is relevant to adults' health.
...
Drinking and smoking ratios are displayed on the US regional map. A map function is created to map variables.
...
For those rarely occured diseases the regional level percentages of specific categorie are quite meaningful. This section of analysis displays some examples of veriations among the 4 regions. Maps in this section are based on continental state multipolygon vector geometry data from US Census Beaureu website. States in the same region are shown in the same color that indicate the percentage values of the region. 
...
All the diseases have binary values and logistic models are  intuitive methods. Only age and BMI are kept to be numeric variables. Factors are first transformed into binary variables before the estimation of models. Baseline categories are the ones that omitted in the estimation results. 

## 5.1. Variables Significantly Correlated to Health
Variables/categories of variables that are significantly correlated to diseases are identified with logistic estiamtes.
...
>The heatmap was designed to display the significant correlations (by the standard of p-values <=0.05) between diseases in columns and predictors in rows. Green in the the above heatmap is used to indicate negatively correlated to diseases and thus positively correlated to health. Red is for positively correlated to diseases and thus negatively related to health. The color scheme shown in the color key is realized using a truncated log-transformed p-values and the sign of its corresponding coefficients. There are 4 steps to transform the p-values: (1). Log-transform the p-values from the estimation; (2). Remove insignificant ones by replacing cells with NaN when corresponding p-values >= 0.05; (3). Add the sign of the corresponding estimated coefficients; (4). Truncate large values by +log(0.0001)  and -log(0.0001) so that colors display in a right range.

## 5.3. Implications of Correlations between Predictors and Health 
A continuous variable or a category of a factor is defined to be "much" healthier if the difference between the number of models in which it is negatively significant in 10 or more above the number of models in which it is positively significant.  A continuous variable or a category of a factor is defined to be healthier if the numbers of models in which it is negatively significant is 5 or more above the number of models in which it is positively significant.  A continuous variable or a category of a factor is defined to be moderately healthier if the number of models in which it is negatively significant is 4 or less above the number of models in which it is ositively significant. If opposite of the three situations are true then they are defined as much worse, worse or moderately worse correspondingly. 
...
All predictors are included in the estimation of the prediction models for the 16 diseases. Records with missing values are excluded in the model training process. About 7.6% of records been removed because of missing values. 
...
Please be patient, the training may take several minutes or longer depending on the environment it runs.

### 6.1.1. Estimation and Cross-Validation 
```{r, ch6.1.1, message=FALSE,warning=FALSE} 
j=1
pred.y.logit <- NA
obs.y.logit <- NA 
pred.y.rf <- NA
...
In this section, the top 10 variables/categories from the full models are used for the model training with 10-fold cross-validation. AUC and ROC are produced to compare the power of predictions.

### 6.2.1. Reduced Model Cross-Validation 
```{r,ch6.2.1,  message=FALSE,warning=FALSE}
j=1
pred.y.logit10 <- NA
obs.y.logit10 <- NA 
pred.y.rf10 <- NA
...
for (j in 1:16) {
  ...
  set.seed(1234)
  # randomly select one of the K from N, the number of rows of data
  s = sample(1:K, size = N, replace = T) # used for label rows for each group       
  pred.outputs.logit <- vector(mode = "numeric", length = N)
  obs.outputs.logit <- vector(mode = "numeric", length = N)
  offset <- 0 # offset is a pointer used for determine where to pending results for the next loop	
...
>Area Under the Curve (AUC) of the logistic regression for all the 16 full models are larger than their corresponding ones from the Random Forest models. 

## 6.3., Reduced vs. Full Model Predictions
If only top 10 variables/catigories of variables are used, the prediction models can be applied for low-cost surveys. This section compares the similarity of full and top 10 variable logistic models.
...
>The prediction qualities are similar when measured by AUC values of the 2 groups of models. Estimtations of alcohol_drug abuse model with both full and top 10 variables are not converged. If ignore the unconverged results for model 15, the reduced models are as good as full model in prediction.

# 7. Discussion about Adding Parent Health as a Predictor
In the previous section the parents diseases have shown strong correlations with adults diseases. But the estimations performed with the code below cannot find many converged coefficients. The reason is the numbers of adults who have parents information are too small to contain reasonable numbers of cases for diseases. Thus, this part of the unconverged results was not reported. 
...
>The last chunk of R code is inactivated because  most of the estimations are not converged. if readers want to check the results, please decomment the last several commented lines of commands such as "x =  tryfunc()", and "pval <- comlstVect(x)". 

# 8. Rare Events and Power of Predictions
Unfortunately, we have unbalanced data, and most diseases occure rarely in the population. Thus the true negative in the confusion table is large. This is a situation when sensitivity is large, precision is small. Such a situatioin implies that if the prediction rule of the logistic models capture most of the diseases, there would be a larger number of false positive predictions even when the rate of error is small and specificity is large. In this section, F-score is constructed as the harmonic mean of sensitivity and precision for a sequence of prediciton rules determined by predicted probability. The best prediction rule corresponding to the maximum F-score for each model is found and reported in this section.
...
> Fourteen out of 16 diseases have rates of occurence less than 3%. 
...
> The best F-score for these models are less than 0.5. About half of the models have only F-scores equal to or less than 0.1.

# 9. Discussion of Rare Event Models

The low rates of diseases in the general US population imply the dificulty of prediction with such unbalanced data. An alternative is to estimate Rare Event Models which use only the cases and selected proportion of controls from the original survey data. The estimates are adjusted for such data selection. The method used for such models is called Rare Event Logistic Model.

## 9.1. Rare Event Full Models
Five controls are randomly selected for each of the cases. R package "Zelig" can estimate rare event logistic models with the selected records. Variables/categories of variables that are significantly correlated to diseases are identified with the rare event logistic model estimation. 
...
>Included in the table above are only significant coefficents from the estimation of full rare event models with all variables. These significant coefficients can also be respresented on a heeat map.
...
The top 10 most significant variables from section 9.2 are selected for the predictions with rare even model. ROC and AUC are produced to assess the quality of prediction
...
>Even for the predictions of the selected data, the maximum F-scores (MaxFscore) are mostly around 0.5, and the sensitivity is still about 0.3, and specificity about 0.9 for most of the models. but these data do not reflect the survey population. Prediction models for the population should be validated with all the data as the next section will do.

### 9.3.2. Prediction of All Data, Top-10 Variable Rare Event Logistic Model 
In this sectiion, a test dataset used in the cross validation of the top-10 variable rare event logistic models is a combinations of the test data as in section 9.3.1. and 1/10 of the unmatched data. Thus the predictions from the 10-fold cross validation in this section represent all the population. The models are still trained with 10-folds of train data as parts of the matched subsets of data, the same as those in scetion 9.3.1. The models take a parameter tau from the combined test data. Please see variables, "top10nam", "ex.data","e" and command lines with "zelig()" for the method used. when controls are selected with the nearest match, most models do not converge. Models trained with randomly matched controls perform better than nearest match. 
...
When the health of all the surveyed adults are predicted with the rare event models, the F scores and sensitivity values are even smaller than those from the full data logistic model in section 9.1. 

# 10. Conclusion
The project has tabled, plotted, mapped and modeled variations of diseases and covariates of interviewed U.S. adults. Logistic models were estimated to quantitively capture these relations. The results are displayed on heatmaps or summarized in tables. The model estimation results are similar to the exploratory analyses. Major conclusions include
1. Smoking is a negative factor for health and strongly correlated with most types of diseases;
2. Physical activities are positive for health and negatively correlated with most diseases;
...
6. There are some regional differences among drinking, smoking, and selective diseases;
7. Prediction models have high values of AUC, but the F scores for the prediction models are very low for most of the diseases because of the rareness of the diseases, or the imbalanced data. Thus, the prediction models may predict too many false cases or not catch true cases. However, for a deadly disease, a positive prediction with a low precision can be considered as an alarm . Further medical examination should be performed.
8. Rare event models cannot predict disease risks better than logistic models.

# 11. Future Improvement Possible
Although plots show that adults' diseases share some similarities with their parents, only 754 adults in the sample have parents information, which is not big enough for modeling the correlation between diseases of adults and their parents.

The models are estimated based on existing cases and predictors without any scientific knowledge of causality. Thus the models do not reflect causality between predictors and disease. The significant predictors are only potential candidates of causes. They may also be the results of lifestyle changes because of the awareness of risk of diseases. Although these models could predict known diseases well, they may not perform as well for predictions of risks of unknown cases as they do for known diseases.  

Propensity score matching methods could be combined with rare event logistic estimation and may produce consistent estimate with a relatively smaller number of matched controls. 

# 12. Aknowledgement 

I would like to give special thanks to Dr. Blanca Himes, at the Institute of Biomedical Informatics of University of Pennsylvania. R methods and code are mostly based on the practicums of her Data Science for Biomedical Informatics class. This project also received help from Dr. Stephen Kawut, Dr. Rui Feng, and Biostatistician Leigh Cressman at the Center for Clinical Epidermiology and Biostatistics of the Perelman School of Medicine, University of Pennsylvania. 
