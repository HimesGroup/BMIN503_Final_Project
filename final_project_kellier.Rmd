...
ha_factor_names <- str_subset(names(ha_data),".factor") %>% str_remove(., ".factor")

image_factor_names <- str_subset(names(imaging_data),".factor") %>% str_remove(., ".factor")

dt_ha_data <- ha_data %>%
  filter(record_id <= nobs) %>%
  select(-all_of(ha_factor_names)) %>%
...
dt_imaging <- imaging_data %>% 
  ...
  summarise(finding = sum(finding, na.rm=T),
...
dt <- dt_demo %>%
  ...
  left_join(dt_ha_data %>% select(-redcap_repeat_instance), by = "record_id") %>%
