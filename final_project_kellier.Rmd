...
dt <- fabricate(N = nobs,
                ...
                female = correlate(given = age, rho = 0.2, draw_binary, prob = 0.55),
                outcome = correlate(given = age, rho = -0.6, draw_binary, prob = outcome_freq[[1]]),
                image = correlate(given = outcome, rho = 0.9, draw_binary, prob = image_freq[[1]]),
                symptoms_mu = pmin(pmax(rnorm(N, mean = 0.05 + 0.2*image + 0.7*outcome - 0.1*age, 
                                              sd = 0.05 + 0.05*image + 0.05*outcome),0),1))
...
dt <- bind_cols(dt, dt_symptoms) %>% 
  ...
  dplyr::mutate(across(c("image", "outcome"), as.factor))
...
dt %>%
  group_by(image, outcome) %>%
  summarise(n_total = n(),
            n_girls = sum(female == 1),
            n_boys = sum(female == 0),
            across(starts_with("symptom"), mean))
...
trim_vars <- c("ID", "image", "outcome")

dt_rf_predictors <- select(dt_train, -trim_vars) %>% data.matrix()

dt_rf <- randomForest(x = dt_rf_predictors, y = dt_train$outcome, ntree = 500, 
                      importance = TRUE, cutoff = c(0.95, 0.05), nodesize = 3)

dt_rf_pred <- predict(dt_rf, select(dt_test, -trim_vars), type = "response")

table(Outcome = dt_test$outcome, Predicted = dt_rf_pred)

ruleExec <- extractRules(RF2List(dt_rf), X = dt_rf_predictors, digits = 2)

ruleMetric <- getRuleMetric(ruleExec, X = dt_rf_predictors, target = dt_train$outcome) %>%
  pruneRule(., X = dt_rf_predictors, target = dt_train$outcome)

learner <- buildLearner(ruleMetric, X = dt_rf_predictors, target = dt_train$outcome, minFreq = 0.05)

pretty_learner <- presentRules(learner, colnames(dt_rf_predictors))

bind_cols(true = dt_test$outcome, pred = applyLearner(learner, X = select(dt_test, -trim_vars))) %>%
  group_by(true, pred) %>%
  summarise(n = n())
