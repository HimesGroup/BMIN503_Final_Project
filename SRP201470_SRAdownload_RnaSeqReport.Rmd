...
```{r var, echo=T}
out_dir <- '/projects/RNASeq/SRP201470/SRP201470_SRAdownload/'
...
if (!exists("pheno_fn")) {
  if(exists("geo_id")){
    ...
    cols=c("title","geo_accession", "characteristics_ch1.2", "characteristics_ch1","files")
    pheno <- pheno.raw %>%
      dplyr::select(cols) %>%
      dplyr::mutate(SRA_ID=files) %>%
      dplyr::mutate(GEO_ID=geo_accession) %>%
      dplyr::mutate(Donor=characteristics_ch1.2) %>%
