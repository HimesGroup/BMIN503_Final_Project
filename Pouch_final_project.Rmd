...
The three faculty members consulted for this project include:
...
GitHub project repository: https://github.com/apouch/BMIN503_Final_Project
...
m6 <- ggplot(data=df.merge, aes(x=sga_10th,y=pappamom, fill=sga_10th)) +
        geom_boxplot() +
        labs(x="sga_10th",y="PAPP-A level") +
        theme(legend.position="none")
...
Since an outlier is noted in the maternal systolic blood pressure (SBP), the data frame _df.merge_ is updated to change the value of this outlier to NA.

``` {r eval=TRUE, warning=FALSE}

df.merge$sbpscrn <- lapply(df.merge$sbpscrn,function(x){if (x > 750) NA else x})

m <- ggplot(data=df.merge, aes(x=sga_10th,y=sbpscrn, fill=sga_10th)) +
        geom_boxplot() +
        labs(x="sga_10th",y="SBP") +
        theme(legend.position="none")
plot(m)
...
## Placenta shape feature selection 
...
pvals.glm <- data.frame("variable"=measures3D.wanted[measures3D.wanted!="sga_10th"],"pvals"=pvals)
...
shape.vars.sig.cor = c("surf_diameter_min","surf_diameter_mean","surf_diameter_max","medial_surface_height_max","edge_minheight","edge_maxheight","SA_fetal","Vsnap","Tcmrep")
...
print(data.frame("placenta measure"=shape.vars.sig.cor,"variance"=shape.vars.variance) %>%
...
df.merge[clinical.vars.wanted][!sapply(df.merge[clinical.vars.wanted],is.factor)] <- as.data.frame(lapply(df.merge[clinical.vars.wanted][!sapply(df.merge[clinical.vars.wanted],is.factor)], normalize))
...
vars.all.cor.reduce = c(shape.cor.reduce,vars.clinical.cor.reduce)
...
model.shape.only <- model.eval(df.vars.shape.only)
model.clinical.only <- model.eval(df.vars.clinical.only)
