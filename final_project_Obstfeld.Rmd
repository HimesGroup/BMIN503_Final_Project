...
kable(rs.p.ptt,col.names = c('SST','SSE for LM', 'r^2 for LM','SSE for RF','r^2 for RF'))
```


A plot of residuals was created as well.
```{r}  
ggplot(data=predictions.p.ptt,aes(obs_outputs)) +
...
kable(rs.p.pt,col.names = c('SST','SSE for LM', 'r^2 for LM','SSE for RF','r^2 for RF'))
```
...
Histograms of results distributions wer again explored.
```{r}
library(gridExtra)
myhists <- list()  # new empty list
...
##Univariate correlations for CHOP data and PTT
...
knitr::kable(arrange(lmres.chop.ptt,variable))
```

```{r}
ggplot(lmres.chop.ptt, aes(x=beta,y=arsquare,color=log,label=variable))+
  geom_point()+
  geom_text_repel(hjust = 0, nudge_x = 0.06,show.legend = FALSE)+
  theme_bw()
...
#Get Univariate correlations for CHOP data PT
```{r}
...
knitr::kable(arrange(lmres.chop.pt,variable))

```

```{r}
ggplot(lmres.chop.pt, aes(x=beta,y=arsquare,color=log,label=variable))+
  geom_point()+
  geom_text_repel(hjust = 0, nudge_x = 0.06,show.legend = FALSE)+
  theme_bw()
...
kable(rs.c.pt,col.names = c('SST','SSE for LM', 'r^2 for LM','SSE for RF','r^2 for RF'))
```
...
The data demonstrate fairly good performance when modeling PT values with a LM model R^2 of 0.47 and RF model R^2 of 0.65.
On the other hand the data demonstrate somewhat poorer performance than what was seen in with Penn data with a LM model R^2 of 0.36 and RF model R^2 of 0.45 when modeling PTT.  
...
Finally the performance of moels created with data from each institutionw as tested against the data from the opposing institutitn.
```{r}
ptt.df<-penndf.ii
      ...
      rf.p.ptt <- randomForest(PTT ~ (FactorVIII)+(FactorIX)+(FactorXI)
                         +(FactorX)+FactorII+FactorV+FactorXII
                         , ntree=1000, data=ptt.df , na.action = na.omit)

   lm.pt <- lm(PT ~ log(FactorVII)+log(FactorX)+log(FactorII)+log(FactorV), data=train)
    rf.pt <- randomForest(PT ~(FactorVII)+(FactorX)+(FactorII)+FactorV,na.action = na.omit, data=pt.df, ntree=100)
```


#Predict PTT for CHOP data on cliassifer trained on  HUP data
```{r}

            
chopdf$facna<-rowSums(apply(is.na(chopdf[,c(4,7,9)]), 2, as.numeric))
chopdf.ii<-chopdf%>%filter(facna<3)%>%filter(!is.na(PTT))%>%data.frame
chopdf.ii[is.na(chopdf.ii)]<-rnorm(mean = 90, sd=10, n=length(chopdf.ii[is.na(chopdf.ii)]))            

ptt.lm.pred<-predict(lm.ptt,chopdf.ii)
ptt.rf.pred<-predict(rf,chopdf.ii)


chop.pred.ptt<-data.frame(chopdf.ii$PTT, ptt.lm.pred,ptt.rf.pred)
chop.pred.ptt<-chop.pred.ptt%>%
  mutate(residual.lm =  ptt.lm.pred - chopdf.ii.PTT,
  residual.rf =  ptt.rf.pred - chopdf.ii.PTT,
  better=ifelse(abs(residual.lm)>abs(residual.rf),'Random Forest','Linear Model'))%>%na.omit


rs.chop.ptt <- chop.pred.ptt %>%
  summarise(
 sst = sum((chopdf.ii.PTT - mean(chopdf.ii.PTT)) ^ 2),
      sse.lm = sum(residual.lm^2),
      r.squared.lm = 1 -( sse.lm / sst),
      sse.rf = sum(residual.rf^2),
      r.squared.rf = 1 - (sse.rf / sst))
...
#Predict PT for CHOP data on cliassifer trained on  HUP data
```{r}

chopdf$facna<-rowSums(apply(is.na(chopdf[,c(3,5,6,8)]), 2, as.numeric))
chopdf.ii<-chopdf%>%filter(facna<4)%>%filter(!is.na(PT))%>%data.frame
chopdf.ii[is.na(chopdf.ii)]<-rnorm(mean = 90, sd=10, n=length(chopdf.ii[is.na(chopdf.ii)]))

pt.lm.pred<-predict(lm.pt,chopdf.ii)
pt.rf.pred<-predict(rf.pt,chopdf.ii)



chop.pred.pt<-data.frame(chopdf.ii$PT, pt.lm.pred,pt.rf.pred)
chop.pred.pt<-chop.pred.pt%>%
  mutate(residual.lm =  pt.lm.pred - chopdf.ii.PT,
  residual.rf =  pt.rf.pred - chopdf.ii.PT,
  better=ifelse(abs(residual.lm)>abs(residual.rf),'Random Forest','Linear Model'))%>%na.omit


rs.chop.pt <- chop.pred.pt %>%
  summarise(
 sst = sum((chopdf.ii.PT - mean(chopdf.ii.PT)) ^ 2),
      sse.lm = sum(residual.lm^2),
      r.squared.lm = 1 -( sse.lm / sst),
      sse.rf = sum(residual.rf^2),
      r.squared.rf = 1 - (sse.rf / sst))
