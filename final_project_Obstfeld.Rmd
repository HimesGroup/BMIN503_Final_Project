...
ptt.df<-penndf[!with(penndf,is.na(FactorVIII)|is.na(FactorXI)|is.na(FactorIX)),]
  ...
  for(i in 1:K){
      ...
      #LM train/test
      lm <- lm(PTT ~ log(FactorVIII)+log(FactorXI)+log(FactorIX), data=train)
      lm.pred.curr <- predict(lm, test)
      pred_outputs.lm[1:length(ptt.df$s[ptt.df$s==i]) + offset] <- lm.pred.curr
  
      #RF train/test
      rf <- randomForest(PTT ~(FactorVIII)+(FactorXI)+(FactorIX),ntree=100, data=train,na.action = na.omit)
...
chopdf<-chop.fc%>%spread(task_name,result)
...
rf <- randomForest(PTT ~(FactorVIII)+(FactorIX)+(FactorXI), data=ptt.df, ntree=100,na.action = na.omit)
 ...
 lm.pred.chop <- predict(lm, chopptt.df)
 rf.pred.chop <- predict(rf,newdata=chopptt.df)

 
chop.pred.ptt<-data.frame(chopptt.df$PTT, lm.pred.chop,rf.pred.chop)
chop.pred.ptt<-chop.pred.ptt%>%
  mutate(residual.lm =  lm.pred.chop - chopptt.df.PTT,
  residual.rf =  rf.pred.chop - chopptt.df.PTT,
  #residual.rsvm = pred_outputs.rsvm - obs_outputs,
  better=ifelse(abs(residual.lm)>abs(residual.rf),'Random Forest','Linear Model'))%>%na.omit


rs.chop.ptt <- chop.pred.ptt %>%na.omit()%>%
  summarise(
    meanobs=mean(chopptt.df.PTT),
    sst = sum((chopptt.df.PTT - meanobs) ^ 2),
    sse.lm = sum(residual.lm^2),
    r.squared.lm = 1 -( sse.lm / sst),
    sse.rf = sum(residual.rf^2),
    r.squared.rf = 1 - (sse.rf / sst))
...
ggplot(data=chop.pred.ptt,aes(chopptt.df.PTT,rf.pred.chop)) +geom_point()
  geom_hline(yintercept = 0) +
  geom_point(aes(y=residual.lm),color='blue',alpha=0.3) +
  stat_smooth(method = "loess",aes(y=residual.lm),color='blue',fill='blue') +
  geom_point(aes(y=residual.rf),color='red',alpha=0.5) +
