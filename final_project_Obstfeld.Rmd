...
ptt.df<-penndf.ii
  ...
  accession<-vector(mode="numeric", length=N)
  offset <- 0
  ...
  for(i in 1:K){
      ...
      accession[1:length(ptt.df$s[ptt.df$s==i]) + offset] <- test$accession
  ...
  predictions.ptt<-data.frame(accession,obs_outputs,pred_outputs.lm,pred_outputs.rf)
...
penndf.ii<-penndf%>%filter(facna<4)%>%filter(!is.na(PT))%>%data.frame
penndf.ii[is.na(penndf.ii)]<-rnorm(mean = 90, sd=10, n=length(penndf.ii[is.na(penndf.ii)]))
...
load(chop)
chop<-cdwdata
names(chop)[7]<-'task_name'
names(chop)[1]<-'empi'
names(chop)[2]<-'col'
names(chop)[15]<-'result'
...
empi<-chop%>%filter(!task_name %in% c('PTT','PT','Fibrinogen'))%>%select(empi)
...
library(tidyr)
library(lubridate)
#Remove results thatfrom the same collection data/time that are discordant,ones that areconcortant, average together
...
remresults<-dblresults%>%filter(cv>0.1)
chop.fc<-anti_join(chop.fc,remresults,by=c('col' = 'col', 'empi' = 'empi'))
...
fac.day<-chop.fc%>%mutate(colday=as.Date(substr(col,1,10),format='%Y-%m-%d'))%>%filter(!task_name %in% c('PTT','PT','Fibrinogen'))
...
#chop.fc contains just screening tests that were drawn on the same day as a factor test, down to 60k tests. Now need to look at dup test by date to facilitate the spread. Will remove any days worth of data if there were greater than 2 test results on that day or if the cv for the day was greater than 10%
dblresults<-chop.fc%>%group_by(empi,colday,task_name)%>%summarise(n=n(),mean=mean(result),stdev=sd(result),cv=sd(result)/mean(result))%>%filter(n>1)
remresults<-dblresults%>%filter(cv>0.1 | n>2)
chop.fc<-anti_join(chop.fc,remresults,by=c('colday' = 'colday', 'empi' = 'empi',"task_name" = "task_name"))
chop.fc<-chop.fc%>%ungroup%>%group_by(empi,colday,task_name)%>%summarise(result=mean(result))
chop.fc%>%group_by(empi,colday,task_name)%>%summarise(n=n())%>%filter(n>1)
```
...
library(gridExtra)
myhists <- list()  # new empty list
for (i in 1:length(colnames(chopdf))) {
    p1 <- eval(substitute(
...
chopptt.df<-chopdfxx[!with(chopdfxx,is.na(FactorVIII)| is.na(FactorIX)| is.na(FactorXI)| is.na(PTT)),]
```

#Get Univariate correlations for CHOP data PTT
```{r}

attach(chopdf)
chopdf.lm<-chopdf%>%ungroup%>%select(3:13)
#perofrm lm and store as list
...
ptt.df<-chopdf.ii
  ...
  for(i in 1:K){
      ...
      #LM train/test
      lm <- lm(PTT ~ log(FactorVIII)+log(FactorXI)+log(FactorXII)+log(FactorIX), data=train)
      lm.pred.curr <- predict(lm, test)
      pred_outputs.lm[1:length(ptt.df$s[ptt.df$s==i]) + offset] <- lm.pred.curr
  
      #RF train/test
      rf <- randomForest(PTT ~(FactorVIII)+(FactorXI)+(FactorIX)+(FactorXII)
                         +FactorV+FactorII
                         ,ntree=100, data=train,na.action = na.omit)
      rf.pred.curr <- predict(rf, newdata=test) 
      pred_outputs.rf[1:length(ptt.df$s[ptt.df$s==i]) + offset] <- (rf.pred.curr)
...
#Create one unified data
```{r}
penndf.u<-penndf%>%ungroup%>%select(-c(accession,allna,ReptilaseTest))%>%mutate(hos='HUP')
chopdf.u<-chopdf%>%ungroup%>%select(-colday)%>%mutate(hos='CHOP')

df.u<-union(penndf.u, chopdf.u)
df.u$hos<-as.factor(df.u$hos)
```


#Impute union data for PT
```{r}
df.u$facna<-rowSums(apply(is.na(df.u[,c(2,4,5,7)]), 2, as.numeric))
df.u.ii<-df.u%>%filter(facna<4)%>%filter(!is.na(PT))%>%data.frame
df.u.ii[is.na(df.u.ii)]<-rnorm(mean = 90, sd=10, n=length(df.u.ii[is.na(df.u.ii)]))

```


#Train and cross validate multivariate models for PT Union
```{r}
library(modelr)
library(purrr)
library(broom)
...
#Impute Union data for PTT
```{r}


df.u$facna<-rowSums(apply(is.na(df.u[,c(3,6,8)]), 2, as.numeric))
df.u.ii<-df.u%>%filter(facna<2)%>%filter(!is.na(PT))%>%data.frame
df.u.ii[is.na(df.u.ii)]<-rnorm(mean = 90, sd=10, n=length(df.u.ii[is.na(df.u.ii)]))
...
ptt.df<-df.u.ii
  ...
  for(i in 1:K){
      ...
      #LM train/test
      lm <- lm(PTT ~ log(FactorVIII)+log(FactorXI)+log(FactorXII)+log(FactorIX)
               +log(FactorV)+log(FactorII)+log(FactorX)
               , data=train)
      lm.pred.curr <- predict(lm, test)
      pred_outputs.lm[1:length(ptt.df$s[ptt.df$s==i]) + offset] <- lm.pred.curr
...
#Build model off Penn data and test against chop data
```{r}
ptt.df<-penndf.ii
      lm.ptt <- lm(PTT ~ log(FactorVIII)+log(FactorXI)+log(FactorIX) +log(FactorXII)+log(FactorII)+log(FactorV)+log(FactorX)
               , data=ptt.df)
      rf <- randomForest(PTT ~ (FactorVIII)+(FactorIX)+(FactorXI)
                         +(FactorX)+FactorII+FactorV+FactorXII
...
chopdf.ptt<-penndf%>%select(4,8,10,14)
