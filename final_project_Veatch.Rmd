...
library(reshape2)
library(ggplot2)
#How many ASD candidate genes fall into categories of interest: 
...
ASDgenebrainexpression<-as.matrix(ASDgenebrainexpression)
coolmap(ASDgenebrainexpression, cluster.by = "de pattern", linkage.row = "none", linkage.col = "none", show.dendrogram = "none", 
        main = "Brain Region Expression Normalized by ASD Gene", xlab="Brain Region", ylab="ASD Candidate Genes", keysize=0.75,  offsetCol = 0, margins = c(12, 5))
```  

``` {r eval=TRUE}
#What are the most common mouse phenotypes of interest?
```    
