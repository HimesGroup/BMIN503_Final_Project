...
+Next, the list of ASD candidate genes was evaluated to identify genes that are specifically expressed in the tissue of most interest to ASD etiology (i.e., the human brain). Details for identifying genes with baseline expression in human tissue from GTEx are provided here: https://www.ebi.ac.uk/gxa/help/r-data-objects.html, here http://www.bioconductor.org/packages/release/bioc/html/ExpressionAtlas.html, and here https://www.ebi.ac.uk/arrayexpress/experiments/E-MTAB-5214/. Optional code is provided below if the user prefers. As the direct pull for all GTEx data is large and we were only interested in brain regions, data for genes expressed at the GTEx default cut-off of FPKM≥0.5 in all human brain regions available were downloaded directly.
``` {r eval=TRUE}
...
``` {r eval=TRUE}
#How many ASD candidate genes fall into categories of interest: 1) expressed in human brain, 2) have relevant mouse phenotype when knocked out, 3) are FDA-approved drug targets?
