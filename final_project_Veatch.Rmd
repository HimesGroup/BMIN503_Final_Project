...
ASDGeneAttributes.melt<-melt(as.data.frame(ASDGeneAttributes), id.vars = "ASDRiskGene")
...
ggplot(data=ASDGeneAttributes.melt, aes(x=variable, y=value, fill=variable)) + 
      ...
      labs(fill = "Functional Annotation")
...
ASDgenebrainexpression <- inner_join(ASDbraingenes.mousepheno.drugtarget[, c(1, 5)], GTExbraindata, by="Gene.Name")
ASDgenebrainexpression <- ASDgenebrainexpression %>%
    select(as.numeric(c(4:16)))
ASDgenebrainexpressionm<-as.matrix(ASDgenebrainexpression)

ASDgenebrainexpressionTEST<-ASDbraingenes.mousepheno.drugtarget %>%
...
#Determine the ASD candidate genes wtih the strongest expression in the human brain
...
ASDgenemousekos <- inner_join(ASDbraingenes.mousepheno.drugtarget[, c(1, 5)], GTExbraindata, by="Gene.Name")
  select(, na.exclude=TRUE)
