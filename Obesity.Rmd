...
race.data <- data %>% 
  ...
  group_by(RACE2)
race.data$NUMADULT <- as.numeric(as.character(race.data$NUMADULT))
...
# household size
kids <- race.data %>%
  select(Race = RACE2, Kids = TOTKIDS) %>%
  na.omit
ggplot(kids, aes(Kids)) +
  geom_bar(aes(fill = Race))

adults <- race.data %>%
  select(Race = RACE2, Adults = NUMADULT) %>%
  na.omit
ggplot(adults, aes(Adults)) +
  geom_bar(aes(fill = Race))

size <- race.data %>%
  select(Race = RACE2, Kids = TOTKIDS, Adults = NUMADULT) %>%
  na.omit %>%
...
# insurance rate
insurance.data <- race.data %>%
  select(Race = RACE2, Insured = INSURED) %>%
  na.omit
ggplot(insurance.data, aes(Race)) +
  geom_bar(aes(fill = Insured), position = "fill") +
  labs(y = "Proportion")

# usual go
usual <- race.data %>%
  select(Race = RACE2, Usual = USUALGO) %>%
  na.omit
...
# overall health
health <- race.data %>%
  select(Race = RACE2, Health = HEALTH) %>%
  na.omit
ggplot(health, aes(Race)) +
...
ggplot(asthma, aes(Race, Count)) +
  geom_bar(stat = "identity", position = "fill", aes(fill = Asthma)) +
  labs(y = "Proportion")
ggplot(asthma, aes(Asthma, Count)) +
